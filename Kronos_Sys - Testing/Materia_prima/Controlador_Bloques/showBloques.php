...
$resultado = $MySQLiconn->query("SELECT idSustrato,descripcionSustrato,codigoSustrato,anchura FROM sustrato where baja=1 order by idSustrato desc ");
while ($rows = $resultado->fetch_array()) {
	...
	<td>
		<a href="?lot=<?php echo $rows["idSustrato"]; ?>"><IMG src="../pictures/boxLote.png" title='Lotes de materia prima'></a>
		<a id="excel" href="?excel=<?php echo $rows["idSustrato"]; ?>"><IMG src="../pictures/excel.png" title='subir lotes'></a>
		<a  target="_blank" href="funciones/pdfReporteLotes.php?cdgbloque=<?php echo $rows['idSustrato'];?>&nameSus=<?php echo $rows['descripcionSustrato'];?>"><IMG src="../pictures/reports.png" title='Reporte de lotes'></a>
...
//Traemos la tabla y la imprimimos donde pertenece 
		...
		$TIM=$MySQLiconn->query("SELECT descripcionImpresion as numT,codigoImpresion from impresiones where sustrato='".$rows['idSustrato']."'");
		if($TIM->num_rows>0)
		{
