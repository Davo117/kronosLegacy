...
<div class="panel-heading"><b class="titulo">Confirmaciones de producto por orden compra</b></div>
...
<select name="orden" id="mySelect"   data-live-search="true"  class="selectpicker show-menu-arrow form-control" onChange="myFunction(this.value);">
  <option value="">Seleccionar Orden de Compra</option>
<?php  //Seleccionar todos los datos de la tabla 
$resultado=$MySQLiconn->query("SELECT idorden, orden, nombresuc FROM $tablaOrden INNER JOIN $tablasucursal ON $tablasucursal.idsuc=$tablaOrden.sucFK where bajaOrden='1' order by idorden desc");
...
while ($row = $resultado->fetch_array()) {
  ...
  if(isset($_GET['edit'])){ 
    if($getROW['ordenConfi']==$row['idorden']){    ?>
...
<div class="col-xs-3">
<label for="combillo">Modelo del Producto</label>
...
function mia(str){
  $('#txtHint').load('funciones/mostrar/mostradorOrdenConfir.php?q='+str);
}
