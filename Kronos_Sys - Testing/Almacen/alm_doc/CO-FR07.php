<?php
  ...
  $SQL=$mysqli->query("SELECT (SELECT concat(nombre,' ',apellido) from saturno.empleado where numemple=mv.datoextra1) as empleado,
                (SELECT d.nombre from saturno.departamento d where d.id=(select departamento from saturno.empleado where numemple=mv.datoextra1)) as area,time(en.fecha) as hora,date(en.fecha) as fecha, un.identificadorUnidad as nombreUnidad,pr.producto,mv.cantidad,mv.costo FROM obelisco.salidas en 
...
while ($row=$SQL->fetch_array())
  {
    ...
    $lstmov[$contador]['cantidad']=$row['cantidad'];
  ...
  { $pdf->SetFont('Arial','B',12);
    ...
    $pdf->SetFont('Arial','',7);
    $pdf->Cell(6,8,$lstmov[$index]['unidad'],1,0,'C');
    $pdf->Cell(10,8,number_format($lstmov[$index]['cantidad'],2),1,0,'C');
