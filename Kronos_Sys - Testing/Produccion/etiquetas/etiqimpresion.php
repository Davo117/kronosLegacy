...
$consul=$MySQLiconn->query("SELECT  tblotes.idLote,tblotes.anchuraBloque,tblotes.noop,tblotes.tarima,tblotes.referenciaLote,tblotes.numeroLote,impresiones.id as newid,`tbpro".$proceso."`.longitud as newLong,`tbpro".$proceso."`.peso as newPeso,`tbpro".$proceso."`.fecha as fecha,`tbpro".$proceso."`.unidades as newUnidades,impresiones.id,(select descripcion from producto where ID=impresiones.descripcionDisenio) as descripcionDisenio,impresiones.descripcionImpresion,impresiones.anchoPelicula,impresiones.anchoEtiqueta,impresiones.espaciofusion from impresiones  inner join `tbpro".$proceso."` on  impresiones.id=`tbpro".$proceso."`.producto inner join tblotes on `tbpro".$proceso."`.noop=tblotes.noop where `tbpro".$proceso."`.producto='".$producto."' and tblotes.tipo='$tipo' and `tbpro".$proceso."`.total=1 and `tbpro".$proceso."`.tipo='$tipo'");
if($tipo=="1")
{
...
if ($consul->num_rows > 0)
  { $pdf=new FPDF('P','mm','lbl4x2');
    ...
    { $pdf->AddPage();
      ...
      $pdf->Cell(2,4,'',0,0,'L');
      $pdf->Cell(98,4,utf8_decode($regPackingList->descripcionDisenio),0,1,'L'); 
      $pdf->Cell(2,4,'',0,0,'L');
