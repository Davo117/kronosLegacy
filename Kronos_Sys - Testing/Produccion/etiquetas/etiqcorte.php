...
$consul=$MySQLiconn->query("SELECT p.producto,p.rollo as codigo from `tbprocorte` p inner join tbcodigosbarras c on c.codigo=p.rollo where p.total=1 and p.producto='".$producto."' and p.rollo_padre=0 and c.proceso=6 and c.baja=1");
...
{
  $Pem=$MySQLiconn->query("SELECT alturaEtiqueta,anchoEtiqueta,codigoImpresion,millaresPorPaquete FROM impresiones WHERE id='".$regCodePaquete->producto."'");
  $ras=$Pem->fetch_array();
...
for ($item = 1; $item <= $nPaquetes; $item++)
  {  $pdf->AddPage();
      $pdf->SetFont('Arial','',9);
      $pdf->Cell(53,10,utf8_decode($ancho.'x'.$alto.' '.$codigo),0,0,'C');
      // Código de barras
    $pdf->SetY(10);
    $pdf->SetX(6);
