...
$consul=$MySQLiconn->query("SELECT `tbpro".$procesoActual."`.*,bandaspp.idBSPP as newid,(select anchura from bandaseguridad where IDBanda=bandaspp.identificadorBS) as anchura,(select nombreBanda from bandaseguridad where IDBanda=bandaspp.identificadorBS) as descripcionDisenio,bandaspp.nombreBSPP as descripcionImpresion,bandaspp.anchuraLaminado as alturaEtiqueta from `tbpro".$procesoActual."` inner join bandaspp  where bandaspp.IdBSPP='".$producto."' and `tbpro".$proceso."`.longitud!='' and `tbpro".$proceso."`.total=1 and `tbpro".$proceso."`.tipo='".$tipo."' and `tbpro".$proceso."`.producto='".$producto."' order by `tbpro".$procesoActual."`.noop DESC");
}
...
// Código de barras
      ...
      $codigoBarras=get_code($regPackingList->noop,$tipo,$MySQLiconn);
      // Código de barras
      $pdf->SetY(4);
      $pdf->SetFont('3of9','',34);      
      $pdf->Cell(80,5,'',0,0,'R'); 
