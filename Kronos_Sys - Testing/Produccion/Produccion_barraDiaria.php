...
<div id="barra-lateral" onclick="";
  ">
	...
	<div class="panel panel-info">
		...
		$SQL=$MySQLiconn->query("SELECT  produccion.idProducto,sum(produccion.cantLotes) as lotes,produccion.idtipo, produccion.maquina,produccion.nombreProducto,produccion.disenio,produccion.fechaProduccion,produccion.juegoCilindros,produccion.juegoCireles,produccion.suaje,produccion.tipo,sum(produccion.unidades) as unidades,produccion.juegoLotes from produccion where produccion.estado='2' and produccion.fechaProduccion='".$fechaMostrador."' and produccion.idtipo='".$tipo."' GROUP by produccion.nombreProducto");
...
if($cilinders==1 || $cirels==1)
	{							/*
					...
					&nbsp; <a target="_blank" title="Imprimir" style="margin-right:5px;" href="documentos/pro-fr07.php?cdgimpresion=<?php echo $raw['codigoImpresion'];?>&cdgmaquina=<?php echo $raw['maquina'];?>&fchprograma=<?php echo $row['fechaProduccion'];?>&cdgjuego=<?php if(!empty($cilinders)){ echo $row['juegoCilindros'];}else if(!empty($cirels)){ echo $row['juegoCireles'];}?>&juegolotes=<?php echo $row['juegoLotes'];?>">PRO-FR07 |&nbsp;</a>
					...
					<a target="_blank" style="margin-right:5px;" href="documentos/pro-fr05.php?cdgimpresion=<?php echo $raw['codigoImpresion'];?>&cdgmaquina=<?php echo $raw['maquina'];?>&fchprograma=<?php echo $row['fechaProduccion'];?>&cdgjuego=<?php if($cilinders==1){echo $row['juegoCilindros'];}else if($cirels==1){echo $row['juegoCireles'];}else{ echo $row['suaje'];}?>&millares=<?php echo $row['unidades'];?>">PRO-FR05 |&nbsp;</a> 
...
<?php
				...
				else if($tipo=="1")
				{
					...
					$resultado=$MySQLiconn->query("SELECT l.idLote,l.noop,l.noLote,l.longitud,l.referenciaLote,l.peso,l.anchuraBloque,l.unidades,l.estado from lotes l inner join tbproduccion p on p.juegoLotes=l.juegoLote where p.fechaProduccion='".$fechaMostrador."' and p.nombreProducto='".$row['idProducto']."' and l.estado=1 or p.fechaProduccion='".$fechaMostrador."' and p.nombreProducto='".$row['idProducto']."' and l.estado=5" );
			while($rak=$resultado->fetch_array())//Este guail(xd) agrega los lotes en cada programación diaria
			{
				...
				if($rak['estado']==5)
				{
					...
					?>
					<a style="float:left;" onclick="return confirm('Este lote ya fué procesado'); " ><IMG src="../pictures/gearfourth.png" title='Lote producido'></a>
					<?php
				...
				{
					?>
					<a style="float:left;" href="?return=<?php echo $rak['idLote']; ?>&tipo=<?php echo $row['idtipo'];?>" onclick="return confirm('¿Seguro que desea regresar este bloque?'); " ><IMG src="../pictures/unnamed.png" title='Regresar lote'></a>
					<?php
					print("<div class='lotesDiarios'>");
					print("<b style='font: 100% Sansation;'>NoOP:</b><b style='font:bold 100% Sansation;'>&nbsp;".$rak['noop']."</b>");
					print("<br>No. de lote:&nbsp; <b style='font:bold 100% Sansation;'>".$rak['noLote']."&nbsp; &nbsp; </b>Longitud:<b style='font:bold 100% Sansation;'>".$rak['longitud']."</b><br><b style='font:bold 100% Sansation;'>".number_format($rak['unidades'],3)." &nbsp;</b>".$decision." &nbsp; Anchura: &nbsp;<b style='font:bold 100% Sansation;'>".$rak['anchuraBloque']."</b><br> Referencia: &nbsp; <b style='font:bold 100% Sansation;'>".$rak['referenciaLote']."</b> &nbsp; Peso: &nbsp;<b style='font:bold 100% Sansation;'>".$rak['peso']."</b>");
