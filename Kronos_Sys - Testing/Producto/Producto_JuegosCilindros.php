...
<label for="comboImpresiones">Impresión:</label>
    <select onChange="document.formulary.submit()" class="form-control" name="comboImpresiones"   id="comboImpresiones">
        <option value="">---</option>
<?php
include ("Controlador_Disenio/db_Producto.php");
if(isset($_POST['comboImpresiones']))
{
    $impresion=$_POST['comboImpresiones'];
}
else if(isset($_GET['cil']))
{
    $impresion=$_GET['cil'];
}
...
{
	?>
	<option value="<?php echo $row ['id'];?>" selected><?php echo $row['productos'];?></option>
	<?php 
}
	else{
		?>
...
<input id="diametro" class="form-control" onkeypress="return numeros(event)"  name="diametro" value="<?php if(isset($_GET['edit'])){ echo $getROW['diametro']; if($getROW['prod']==1){echo '"readonly="true';}}?>"
