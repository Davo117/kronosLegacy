...
$resultado = $MySQLiconn->query("SELECT i.id,i.descripcionImpresion FROM impresiones i inner join producto p on p.ID=i.descripcionDisenio inner join tipoproducto t on t.id=p.tipo where i.baja=1 and p.baja=1 and t.baja=1");
?>
...
{
    $resultado = $MySQLiconn->query("SELECT descripcionproceso,numeroProceso from juegoprocesos where identificadorJuego=(SELECT juegoProcesos from tipoproducto where id=(SELECT tipo from producto where ID=(
...
<select required  name="comboElementos" id="comboElementos" class="selectpicker show-menu-arrow form-control" data-style="form-control" data-live-search="true" title="--Selecciona el producto--">
    <?php
...
// CONSULTA QUE TRAE LOS ELEMENTOS AGREGADOS AL MODULO DE ELEMENTO DE CONSUMO//
  $obel=$MySQLiconn->query("SELECT  producto as id, id as codigo from obelisco.productosCK ");
...
?> 
	...
	<?php
if( $row['CIDPRODUCTO']==$mySQL['id'])
{
	?>
	<option data-tokens="<?php echo $row['nombreElemento'];?>" value="<?php echo $mySQL['id'];?>" selected><?php  echo $row['id'];?></option>
	<?php 
}
	else{
		?>
<option data-tokens="<?php echo $row['nombreElemento'];?>" value="<?php echo $mySQL['id'];?>"><?php echo $row['nombreElemento'];?></option>
...
<input type="text" name="cantConsumo" id="cantConsumo"  class="form-control" required value="<?php if(isset($_GET['edit'])) echo $getROW['consumo'];?>" placeholder="Consumo por etiqueta">
</div>
</div>
<?php
