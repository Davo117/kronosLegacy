...
if ($result->num_rows > 0) {   
	...
	$perfiles=$MySQLiconn->query("SELECT cdgmodulo, permiso from accesos where perfil='".$row['perfil']."'");
	$_SESSION['permitido']= array();
	$row_cnt=$perfiles->num_rows;
	if ($row_cnt> 0) {   
		while($rowPer=$perfiles->fetch_array()) { 
			$_SESSION['permitido'][$rowPer['cdgmodulo']]=$rowPer['permiso'];
	 	}
	}
