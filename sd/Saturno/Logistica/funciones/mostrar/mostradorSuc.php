<?php
print("<th>&nbsp Opciones &nbsp</th>");
print("<th>ID</th>");
print("<th>Nombre</th>");
print("<th>Teléfono</th>");

//Mientras que se tengan resultados se le asignan a $rows mediante 

$resultado = $MySQLiconn->query("SELECT * FROM $tablasucursal where bajasuc=1  ORDER BY idsuc DESC;");

while ($rows = $resultado->fetch_array()) {
//Realizamos una acción en donde la imagen de eliminar o modificar mandan a llamar su respectivo documento donde se realiza su proceso:
print("<tr>");
//asignamos el atributo value con el id al que corresponde cada registro
?>
<td> <a href="?edit=<?php echo $rows["idsuc"]; ?>" onclick="return confirm('<?php print("Deseas EDITAR la Sucursal: ".$rows["nombresuc"]." ?"); ?>'); " ><IMG src="funciones/img/modify.png" title='Modificar'></a>

<a href="?del=<?php echo $rows["idsuc"]; ?>" onclick="return confirm('<?php print("Deseas ELIMINAR la Sucursal: ".$rows["nombresuc"]." ?"); ?>');" ><IMG src="funciones/img/delete.png" title='Desactivar'></a>
&nbsp  
<a href="?comboconsuc=<?php echo $rows["nombresuc"]; ?>"><IMG src="funciones/img/contacto.png" title='Contacto Sucursal'></a>

<a href="?comboOC=<?php echo $rows["nombresuc"]; ?>"><IMG src="funciones/img/ordencom.png" title='Orden Compra'></a>
 
<a href="?comboembar=<?php echo $rows["nombresuc"]; ?>"><IMG src="funciones/img/embarque.png" title='Embarques'></a>
 </td>
<?php
//Traemos la tabla y la imprimimos debidamente donde pertenece 
print("<td title='ID'> &nbsp  ".$rows["idsuc"]." &nbsp  </td>");
print("<td title='Nombre'>&nbsp &nbsp   ".$rows["nombresuc"]."&nbsp &nbsp   </td>");
print("<td title='Teléfono'>&nbsp  &nbsp  ".$rows["telefonosuc"]."&nbsp  &nbsp  </td>");
print("</tr>");
}
 

 //Contar registros:
$row_cnt=$resultado->num_rows;
printf("<p id='mostrar'>Se muestran: ".$row_cnt." Registros Activos</p>");
$resultado->close();

?>