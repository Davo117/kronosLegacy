...
if(isset($_POST['save']))
{
	...
	if($_POST['tipocombo']=="Termoencogible")
	{
		$tipo="Termoencogible";
	}
	else 
	...
	$predeterminados;
		...
		$predeterminados=1;
		$consulta =$MySQLiconn->query("SELECT * from consumos where producto='0' and baja=1;");
	...
	$refil=0;
	$fus=0;
	$TI=$MySQLiconn->query("call getJuegoProcesosByType('".$tipo."')");
...
if(isset($_GET['edit']))
{
	$SQL = $MySQLiconn->query("SELECT DISTINCT p.descripcion,p.*,pr.disenio as prod FROM producto p left join produccion pr ON pr.disenio=p.descripcion WHERE p.ID='".$_GET['edit']."'");
