...
if(isset($_GET['save']))
{
	...
	$anchuraLaminado = $MySQLiconn->real_escape_string($_GET['anchuraLaminado']);
	 ...
	 //En caso de ser diferente la consulta:
		if(!$SQL)
		{
	 	//Mandar el mensaje de error
			...
			<strong>Error</strong> ,ocurrió un problema al registrar,intentelo mas tarde
			</div>";
		} else{
...
{
	$SQL = $MySQLiconn->query("UPDATE bandaspp SET identificadorBSPP='".$_GET['identificadorBSPP']."', nombreBSPP='".$_GET['nombreBSPP']."', anchuraLaminado=".$_GET['anchuraLaminado'].",sustrato='".$_GET['comboSustratos']."' WHERE IdBSPP=".$_GET['editado']);
	echo "<script>window.location=Producto_BandasSPP.php;</script>";
