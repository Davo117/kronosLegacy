...
"SELECT DISTINCT jl.identificadorCilindro,jl.*,i.anchoPelicula,i.alturaEtiqueta FROM juegoscilindros jl INNER JOIN impresiones i ON jl.descripcionImpresion=i.descripcionImpresion WHERE jl.baja=1 && jl.descripcionImpresion='$q'");
...
print("<tr>");
?>
<td><a href="?edit=<?php echo $rows["IDCilindro"]; ?>"" ><IMG src="../pictures/modificarproducto.png" title='Modificar'></a>  
	<?php
	if($rab['prod']!=2)
	{
		$inProd=1;?>
    <a href="?del=<?php echo $rows["IDCilindro"]; ?>"><IMG src="../pictures/deletProducto.png" title='eliminar'></a></a> 
    <?php
}
...
$soul=$MySQLiconn->query("SELECT sum(unidades) as desgaste,(SELECT sum(unidades) from proimpresion where juegoCilindros=concat('".$rows["identificadorCilindro"]."',' | ',(SELECT proveedor from juegoscilindros where identificadorCilindro='".$rows["identificadorCilindro"]."'),' D.')) as registrado from produccion where juegocilindros='".$rows["identificadorCilindro"]."'");
