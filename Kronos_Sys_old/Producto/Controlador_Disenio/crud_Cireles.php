...
if(isset($_POST['save'])){
	...
	$id = $MySQLiconn->real_escape_string($_POST['identificadorJ']);
	$diente = $MySQLiconn->real_escape_string($_POST['num_dientes']);
	...
	$cat=$MySQLiconn->query("SELECT s.anchura,i.anchoPelicula,i.espacioRegistro FROM sustrato s INNER JOIN impresiones i
		ON  s.descripcionSustrato=i.sustrato WHERE i.codigoImpresion='".$prod."' ");
...
if(isset($_POST['update'])){
	$cat=$MySQLiconn->query("SELECT s.anchura,i.anchoPelicula FROM sustrato s INNER JOIN impresiones i
		ON  s.descripcionSustrato=i.sustrato WHERE i.codigoImpresion='".$_POST['producto']."'");
	$dim=$cat->fetch_array();
	$tabla=$dim['anchura'];//Aquí estoy igualando la tabla y las repeticiones al paso alas del diseñi ligado
	$repeticiones=$tabla/$dim['anchoPelicula'];
	if(($dim['anchoPelicula']*$repeticiones)==$tabla)
	{
		if($tabla==$tabla)
		{
			...
			$SQL = $MySQLiconn->query("UPDATE juegoscireles SET producto='".$_POST['producto']."', identificadorJuego='".$_POST['identificadorJ']."', num_dientes='".$_POST['num_dientes']."',  ancho_plano='".$tabla."', fecha_entrega='".$_POST['fechaE']."',  observaciones='".$_POST['observaciones']."',repeticiones='".$repeticiones."',alturaReal='".$_POST['alturaReal']."' WHERE id=".$_GET['edit']); 
	//Realizamos la consulta
