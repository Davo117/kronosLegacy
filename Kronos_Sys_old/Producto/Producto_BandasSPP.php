...
<div class="col-xs-3">
<label for="anchuraLaminado">Anchura para laminado:</label>
<input type="text" name="anchuraLaminado" id="anchuraLaminado" class="form-control" value="<?php if(isset($_GET['edit'])) echo $getROW['anchuraLaminado'];?>"
    size="7" placeholder="mm">
</div>

<div class="col-xs-3">
<label for="comboSustratos">Sustrato:</label>
<select name="comboSustratos" class="form-control" id="comboSustratos">
    <?php
include ("Controlador_Disenio/db_Producto.php");
...
{
    ?>
    <option selected value="<?php echo $row ['descripcionSustrato'];?>" ><?php echo $row['descripcionSustrato'];?></option>
    <?php
}
else
{
