...
SELECT DISTINCT p.descripcion,p.tipo,pr.estado,p.fus,p.holograma FROM producto p 
LEFT JOIN produccion pr 
ON pr.disenio=p.descripcion where p.baja=1");
...
if( $row ['descripcion']==$tipodescripcion)
{
	...
	?>
	<option value="<?php echo $row ['descripcion'];?>" selected><?php echo  $row['descripcion'].' ['.$row['tipo'].']';?></option>
	<?php 
}
	else{
		?>
<option value="<?php echo $row ['descripcion'];?>"><?php echo $row['descripcion'].' ['.$row['tipo'].']';?></option>
<?php 
}
} 
?> 
...
if( $getROW['codigoCliente']==$row['nombre'])
{
	...
	<?php 
		...
		?>
<option value="<?php echo $row['nombre'];?>"><?php echo $row['IdentificadorCliente']." |".$row['nombre'];?></option>
<?php 
}
}
...
while($row = $resultado->fetch_array()){
	...
	$rest = $MySQLiconn->query("SELECT* FROM juegoparametros where identificadorJuego=(SELECT juegoparametros from tipoproducto where alias='$e')");
...
<div class="col-xs-3">
<label for="<?php echo $rex['nombreparametro']?>"><?php echo $rex['leyenda'] ?></label>
<input style="float:left;" type="text" name="<?php echo $rex['nombreparametro'] ?>" value="<?php if(isset($_GET['edit'])){ echo $getROW[$valiu]; if($rex['nombreparametro']=='millaresPorRollo' || $rex['nombreparametro']=='porcentajeMPR'){  }else if($inProd==0){}else{ echo '"readonly="true';}} ?>"
    size="18" placeholder="<?php echo $rex['placeholder'] ?>" class="form-control" >
</div>
