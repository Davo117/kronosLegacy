...
{
  $consul=$MySQLiconn->query("SELECT lotes.ancho as anchura,lotes.tipo,lotes.noop,lotes.anchuraBloque,lotes.numeroLote,lotes.referenciaLote,lotes.tarima,produccion.juegoLotes,impresiones.id,impresiones.descripcionDisenio,impresiones.descripcionImpresion,impresiones.anchoPelicula,impresiones.anchoEtiqueta,impresiones.espaciofusion  from produccion inner join lotes on produccion.juegoLotes=lotes.juegoLote inner join impresiones on produccion.nombreProducto=impresiones.descripcionImpresion where produccion.nombreproducto='$producto' and produccion.estado=2 and lotes.estado=5 and lotes.tipo='$tipo'");
}
}
 ...
 if ($consul->num_rows > 0)
  { $pdf=new FPDF('P','mm','lbl4x2'); 
    ...
    { 
       if($proceso=="programado")
    {
      $CAL=$MySQLiconn->query("SELECT unidades,longitud from `pro".$procesoActual."` where tipo='$regPackingList->tipo' and noop='$regPackingList->noop'");
...
else
{
     ...
     $pdf->SetFont('Arial','',10);
      ...
      $pdf->Cell(18,4,'Longitud',0,0,'L');
      $pdf->SetFont('Arial','B',10);
      $pdf->Cell(30,4,number_format($set->longitud,2).' mts',0,1,'R'); 
...
if(!empty($regPackingList->referenciaLote))
{
   $pdf->SetFont('Arial','B',10); 
      $pdf->Cell(52,4,'',0,0,'L');
...
if($proceso!="programado")
{
  $pdf->SetFont('Arial','',10);
      ...
      if($tipo!="BS")
      {
        $pdf->Cell(20,4,number_format($regPackingList->unidades*1000,2),0,1,'R'); 
      }
      else
      {
        $pdf->Cell(20,4,number_format($regPackingList->unidades,2),0,1,'R'); 
      }
}
else
{
   $pdf->SetFont('Arial','',10);
      $pdf->Cell(2,4,'',0,0,'L'); 
      $pdf->Cell(10,0,$decision,0,0,'L');
      $pdf->SetFont('Arial','B',10); 

        $pdf->Cell(30,-1,number_format($set->unidades,2),0,1,'R'); 
}
