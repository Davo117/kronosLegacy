...
if ($_GET==true) {
  ...
  if ($prodE!='--') {
    $consulta1= $MySQLiconn->query("SELECT descripcionDisenio, descripcionImpresion FROM $impresion WHERE  id='".$_GET['prod']."' "); 
    $primera1=$consulta1->fetch_array();
    $prodE= $primera1['descripcionDisenio']."~".$primera1['descripcionImpresion'];
  }else {   $prodE="Todos";   }
...
$confirmar = $MySQLiconn->query("SELECT * FROM $confirProd WHERE embarqueConfi BETWEEN '".$_GET['dsd']."' AND '".$_GET['hst']."'  && bajaConfi='1' ORDER BY embarqueConfi ");    
...
while ($confirmado=$confirmar->fetch_array()){ 
  ...
  if($_GET['suc']!="--"){
    ...
    if($primera['nombresuc']!=''){
      $array="&& sucFK='".$primera['nombresuc']."'"; 
    }
       
    $segundo=$MySQLiconn->query("SELECT orden, sucFK, recepcion FROM $tablaOrden WHERE bajaOrden='1' $array");

    while($segunda=$segundo->fetch_array()){
      $sucursal=$segunda['orden'];
      if ($sucursal==$confirmado['ordenConfi']) {
        $fechas=$confirmado['embarqueConfi'];
        if($_GET['prod']!="--"){
          $primero= $MySQLiconn->query("SELECT descripcionImpresion FROM $impresion WHERE  id='".$_GET['prod']."' "); 
          $primera=$primero->fetch_array();

          $producto=$primera['descripcionImpresion']; 
          if ($confirmado['prodConfi']==$producto) {
            $primera=$MySQLiconn->query("SELECT * FROM $tablaOrden where bajaOrden=1 && orden=(SELECT ordenConfi from $confirProd where bajaConfi=1 && idConfi= ".$confirmado['idConfi'].")");
            ...
            if ($fechar!=$fechas) {  
              ...
              $pdf->SetFont('arial','',8);
              $pdf->Cell(20,4,'Sucursal '.$row1['sucFK'],0,1,'L');
              ...
              $ultima=$MySQLiconn->query("SELECT sum(cantidadConfi) as cantidad FROM $confirProd where bajaConfi='1' && ordenConfi='".$row1['orden']."' && embarqueConfi='".$confirmado['embarqueConfi']."'");
