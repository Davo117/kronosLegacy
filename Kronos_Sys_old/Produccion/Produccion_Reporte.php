...
WHERE i.baja=1 AND p.baja=1 AND p.tipo='".$tipo."'");
    ...
    <div class="col-xs-3">
        <label for="cmbEmpleados">Empleado</label>
    <select name="cmbEmpleados" class="form-control" title="Seleccione un empleado" onchange="document.formulary.submit()">
        ...
        while($rai=$Em->fetch_array())
        {
            ...
            {
                   ?>
            <option value="<?php echo $rai['numEmpleado'];?>"><?php echo $rai['nombre'];?></option>
            <?php
            }
...
<?php
$resultado=$MySQLiconn->query("SELECT descripcionProceso from juegoprocesos where identificadorJuego=(select juegoprocesos from tipoproducto where alias='".$tipo."') and numeroProceso!=0 and registro=1");//Extrae los procesos donde el id del producto sea igual
...
<a  style="float:right;" class="btn btn-large btn-primary" target="_blank"  title="Generar reporte"  href="documentos/historicoLight.php?proceso=<?php echo $_POST['comboProcesos']."&empleado=".$_POST['cmbEmpleados']."&producto=".$_POST['cmbProductos']."&fch=".$_POST['cmbLapso']."&hstfch=".$_POST['cmbfchFinal']."&tipo=".$_GET['tipo']?>"><img class="img-responsive" src="../pictures/reports.png"></a>
</form>
</div>
