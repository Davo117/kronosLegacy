...
{
	$SQL=$MySQLiconn->query("SELECT dato,(SELECT nombreParametro from juegoparametros where numParametro='C' and identificadorJuego=(SELECT packParametros from procesos where descripcionProceso='".$_POST['comboProcesos']."')) as referencial from cache where id=7");
}
else
{
...
$S=$MySQLiconn->query("SELECT operador FROM `pro".$procesoActual."` where $consul $consultProd $consultEmp and `pro".$procesoActual."`.tipo='".$_GET['tipo']."' group by operador");
