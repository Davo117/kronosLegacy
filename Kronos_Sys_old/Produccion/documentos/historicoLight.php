...
class PDF extends FPDF
  ...
  global $prodOperaciones_operacion;
   ...
   $fecha=$_GET['fch'];
      ...
      $this->Cell(75,4,'Histórico de Operaciones',0,1,'L');
 ...
 // if ($prodOperaciones_cdgoperacion == '')
  ...
  if(empty($_GET['proceso']))
  {
    $tipo=$_GET['tipo'];
    $tipos=$tipo;
    $TOP=$MySQLiconn->query("call getJuegoProcesosByType('".$tipo."')");
    $procesos=array();
     $idsProcesos=array();
...
echo "Ref".$dato."--";*/
      $S=$MySQLiconn->query("SELECT DATE(fecha) as fecha,(SELECT divisiones from procesos where descripcionProceso='".$procesoEnCurso."') as divisiones FROM `pro".$procesoEnCurso."` where $consul $consultProd $consultEmp and `pro".$procesoEnCurso."`.tipo='$tipos' group by DATE(fecha)");
...
else
{
  ...
  $SQL=$MySQLiconn->query("SELECT dato,(SELECT nombreParametro from juegoparametros where numParametro='C' and identificadorJuego=(SELECT packParametros from procesos where descripcionProceso='".$_GET['proceso']."')) as referencial from cache where id=7");
$row=$SQL->fetch_object();
...
$dato=$row->referencial;
    $tipos=$_GET['tipo'];
       ...
       $S=$MySQLiconn->query("SELECT DATE(fecha) as fecha,(SELECT divisiones from procesos where descripcionProceso='".$procesoActual."') as divisiones FROM `pro".$procesoActual."` where $consul $consultProd $consultEmp and `pro".$procesoActual."`.tipo='$tipos' group by DATE(fecha)");
while($rem=$S->fetch_object())
{  
