...
if($tipo!="BS"){
  $SQL=$MySQLiconn->query("SELECT numeroProceso from juegoprocesos where identificadorJuego=(select juegoProcesos from tipoproducto where  alias=(select tipo from producto where 
  ...
  $SQL=$MySQLiconn->query("SELECT numeroProceso from juegoprocesos where identificadorJuego=(select juegoProcesos from tipoproducto where alias='BS' and descripcionProceso='".$proceso."')");
  $roa=$SQL->fetch_array();
  $noProces=$roa['numeroProceso'];
}
