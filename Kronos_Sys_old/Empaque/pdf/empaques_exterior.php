...
//Documento aun en fase beta 21/06/2018
  ...
  { 
    ...
    $alptEmpaqueSelect = $MySQLiconn->query("
      SELECT e.codigo,c.proceso 
      FROM ensambleempaques e,codigosbarras c 
      WHERE e.referencia=(SELECT referencia FROM rollo where codigo='".$idPaquete."') and e.producto=(SELECT producto from rollo where codigo='".$idPaquete."' ) and e.codigo=c.codigo");
      ...
      { // Busqueda del empaque entre los empaque de rollo (Banda continua)   
        $packingListSelect = $MySQLiconn->query("
          SELECT `pro".$proceso."`.noop,
            ...
            FROM `pro".$proceso."`,
                 ...
                 ensambleempaques,
                 ".$TipoEmpaque."
            WHERE ensambleempaques.referencia=(SELECT referencia FROM ".$TipoEmpaque." where codigo='".$idPaquete."') and ensambleempaques.producto=(SELECT producto from ".$TipoEmpaque." where codigo='".$idPaquete."' ) and impresiones.descripcionImpresion=".$TipoEmpaque.".producto and  `pro".$proceso."`.noop=(SELECT noop from codigosbarras where codigo=(select codigo from ensambleempaques where referencia=(select referencia from ".$TipoEmpaque." where codigo='".$idPaquete."'))) and rollo.referencia=(SELECT referencia from rollo where rollo.codigo='".$idPaquete."')"); 
        if(!$packingListSelect)
        {
          ...
          SELECT `pro".$proceso."`.noop,
            ...
            FROM `pro".$proceso."`,
                 ...
                 ensambleempaques,
                 ".$TipoEmpaque."
            WHERE ensambleempaques.referencia=(SELECT referencia FROM ".$TipoEmpaque." where codigo='".$idPaquete."') and ensambleempaques.producto=(SELECT producto from ".$TipoEmpaque." where codigo='".$idPaquete."' ) and impresiones.descripcionImpresion=".$TipoEmpaque.".producto and  `pro".$proceso."`.noop=(SELECT noop from codigosbarras where codigo=(select codigo from ensambleempaques where referencia=(select referencia from ".$TipoEmpaque." where codigo='".$idPaquete."')))"); 
        }
        ...
        { $pdf=new FPDF('P','mm','lbl4x2'); 
          ...
          { if (!isset($alptEmpaque_noempaque)) { $alptEmpaque_noempaque = $regPackingList->noempaque; }
            ...
            if (!isset($alptEmpaque_impresion)) { $alptEmpaque_impresion = $regPackingList->diseno.' | '.$regPackingList->impresion; }
...
// Numero de operación,agregado por Erik Castillo
