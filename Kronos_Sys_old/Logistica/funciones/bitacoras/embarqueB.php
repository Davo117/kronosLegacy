...
$surtir=0;
	if ($_GET['emp']=='caja') {
  		$SQL=$MySQLiconn->query("SELECT piezas, referencia FROM caja where producto='".$_SESSION['productillo']."' && baja='2'");
	...
	elseif ($_GET['emp']=='rollo') {
    	$SQL=$MySQLiconn->query("SELECT referencia, piezas FROM rollo where producto='".$_SESSION['productillo']."' && baja='2'");
    	while ($row=$SQL->fetch_array()) {
	      $cdg='number'.$row['referencia'];
    	  $chck = $MySQLiconn->real_escape_string($_POST[''.$cdg.'']);
	      if ($chck!='--') {
