...
if(isset($_GET['del'])){	
	...
	$sqlite = $MySQLiconn->query("SELECT * FROM $tablaOrden WHERE idorden=".$_GET['del']);
	$getROW = $sqlite->fetch_array();	
	if (!empty($getROW['orden'])) {
		$MySQLiconn->query("UPDATE $confirProd SET bajaConfi=0 WHERE ordenConfi='".$getROW['orden']."'");
		$MySQLiconn->query("UPDATE $reqProd SET bajaReq=0 WHERE ordenReqFK='".$getROW['orden']."'");
	}
...
if(isset($_POST['update'])){
	...
	if (!empty($_POST['ordenC'])) {
		...
		$MySQLiconn->query("UPDATE $confirProd SET ordenConfi='".$_POST['ordenC']."' WHERE ordenConfi='".$getROW['orden']."'");
		$MySQLiconn->query("UPDATE $reqProd SET ordenReqFK='".$_POST['ordenC']."' WHERE ordenReqFK='".$getROW['orden']."'");
	}
