...
if(isset($_POST['save'])){
	...
	if ($radio=="0") { 	
		...
		$MySQLiconn->query("UPDATE $embarq SET bajaEmb='0', observaEmb='(EMBARQUE DEVUELTO) ".$row['observaEmb']."'  WHERE numEmbarque='$cdg'");
		$suc=$row['sucEmbFK'];
		$prod=$row['prodEmbFK'];
	}
	...
	if ($radio=="1") { 	
		...
		$suc=$row2['sucEmbFK'];
		$prod=$row2['prodEmbFK'];
		$MySQLiconn->query("UPDATE $empaque SET baja='5', cdgDev='$secuencia' where codigo='".$cdg."'");
