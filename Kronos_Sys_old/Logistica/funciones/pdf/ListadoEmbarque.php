...
if ($_GET['numEmbarque']){ 
  ...
  if ($regVntsEmbarque['producto']!=null) {
    ...
    $vntsSucursalSelect = $MySQLiconn->query("
      SELECT idcliFKS, nombresuc, domiciliosuc, coloniasuc, cpsuc, telefonosuc
          FROM $tablasucursal
          WHERE nombresuc = '".$vntsEmbarque_cdgsucursal."'");
    ...
    if ($vntsEmbarque_cdgempaque == 'rollo') {
      $SLQ=$MySQLiconn->query("SELECT * from rollo where producto='".$regVntsEmbarque['producto']."' && baja='3' && cdgEmbarque='".$_GET['numEmbarque']."' ORDER BY referencia");
      $i=1;
      $item=1;
      ...
      while ($row=$SLQ->fetch_array()) {
        ...
        while ($rowBobina=$bobina->fetch_array()) {
          ...
          if ($pdf->GetX() == $posicionX){ $pdf->Cell(24,5,'Bobina'.' '.$inicial.' ',1,0,'R', true);  } 
          else{ 
            $pdf->Cell(($posicionX-10),4,'',0,0,'R');
            $pdf->Cell(24,5,'Bobina'.' '.$inicial.' ',1,0,'R', true); 
          }
          $pdf->SetFont('arial','',9);
...
$pdf->Ln(-5);
    ...
    $pdf->Output('Lista de embarque '.$vntsEmbarque_cdgembarque.' '.$vntsEmbarque_sucursal.'.pdf', 'I');
  } 
  else{ 
    echo '<!DOCTYPE html>
            <html>
