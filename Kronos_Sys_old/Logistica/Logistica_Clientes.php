...
<input type="text" name="RFC" id="RFC" class="form-control" value="<?php if(isset($_GET['edit'])) echo $getROW['rfccli'].'" readonly="true';  ?>"	size="20" placeholder="#" required>
</div>
...
<input type="text" name="TelefonoCli" class="form-control"  value="<?php if(isset($_GET['edit'])) echo $getROW['telcli'];  ?>"size="20" placeholder="Número Telefónico" required onkeypress="return numeros(event)">
	...
	<?php include ("funciones/mostrar/mostradorCliente.php"); 
echo("</table></div>");

if(isset($_POST['checkTodos'])){?>
	<div class="table-responsive">
	<table  border='0' cellpadding='15' class="table table-hover">
	<?php	include ("funciones/nomostrar/mostrarTodoCliente.php");	?>
	</table>
</div>
	<?php
}?>
