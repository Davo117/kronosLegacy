...
$contador=0;
$resultado = $MySQLiconn->query("SELECT * FROM lotes where bloque='$bloqueDescripcion' && tarima='$tarima'");
...
?>
    ...
    <a href="?produ=<?php echo $rows["idLote"]; ?>" onclick="return confirm('¿Desea liberar el lote<?php echo ' ´'.$rows['referenciaLote'].'´';?>?'); "  ><IMG src="../pictures/factory.png" title='Liberar lote'></a>
    </td> 
<?php
}
...
else if($rows["estado"]==2)
{
    ...
    <a href="?repor=<?php echo $rows["referenciaLote"]; ?>" onclick="return confirm('El lote   <?php echo ' '.$rows['referenciaLote'];?> ya esta liberado,¿desea retornar?')"; ><IMG src="../pictures/verify.png" title='Liberado'></a></td>
    <?php
}
...
{
    ?>
    <td><a href="?acti=<?php echo $rows["idLote"]; ?>"" ><IMG src="../pictures/unnamed.png" title='Modificar'></a>  
    <a href="?delfin=<?php echo $rows["idLote"]; ?>" onclick="return confirm('¿Deseas eliminar definitivamente el lote  <?php echo ' '.$rows['referenciaLote'];?>?'); " ><IMG src="../pictures/definitivo.png" title='Eliminar'></a></td>
    <?php
}
...
if($rows['estado']==0)
{
   echo "<td class='selected' title='Seleccionar lote'><input style='float:right;' type='checkbox' name='check".$contador."' value='".$rows['referenciaLote']."'></td>";
}
else
{
...
?>
<td><a>
    <a href="?edit=<?php echo $rows["idLote"]; ?>"" ><IMG src="../pictures/modificarproducto.png" title='Modificar'></a>  
    <a href="?del=<?php echo $rows["idLote"]; ?>" ><IMG src="../pictures/deletProducto.png" title='calendario'></a>
    <a href="?produ=<?php echo $rows["idLote"]; ?>" onclick="return confirm('¿Desea liberar el lote<?php echo ' ´'.$rows['referenciaLote'].'´';?>?'); "  ><IMG src="../pictures/factory.png" title='Liberar lote'></a>
    </td> 
<?php
...
{
    ?>
    <td><a href="?edit=<?php echo $rows["idLote"]; ?>"" ><IMG src="../pictures/modificarproducto.png" title='Modificar'></a>  
    <a href="?del=<?php echo $rows["idLote"]; ?>" onclick="return confirm('Esta seguro que desea eliminar el lote  <?php echo ' '.$rows['referenciaLote'];?>? !'); " ><IMG src="../pictures/deletProducto.png" title='Eliminar'></a></td>
    <?php
}
else if($rows["estado"]==4)//Eliminación definitiva
...
if($rows['estado']==0)
{
   echo "<td title='Seleccionar lote'><input style='float:right;' type='checkbox' name='check".$contador."' value='".$rows['referenciaLote']."'></td>";
}
else
{
...
$resultado = $MySQLiconn->query("SELECT tarima from lotes where bloque='$bloqueDescripcion' && tarima='$tarima'");
