...
$pesoE5=0;
  ...
  $querySelect = $MySQLiconn->query("SELECT * FROM $bloqueM WHERE sustrato='".$progBloque_cdgbloque."'");

if ($querySelect->num_rows > 0){ 
  $regQuery = $querySelect->fetch_object();
  
  $progbloque_idbloque = $regQuery->sustrato;
  $progbloque_sustrato = $regQuery->sustrato; 
  }
  $querySelect = $MySQLiconn->query("SELECT tarima FROM lotes WHERE bloque = '".$progBloque_cdgbloque."'
  GROUP BY tarima ORDER BY tarima");

  if ($querySelect->num_rows > 0){
    $item = 0;
    $nombreB=$regQuery->sustrato;
    ...
    while ($regQuery = $querySelect->fetch_object()){
      ...
      $subQuerySelect = $MySQLiconn->query("SELECT * FROM lotes WHERE bloque='$nombreB' AND tarima='".$progLote_tarima[$item]."' ORDER BY bloque");
