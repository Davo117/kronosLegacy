...
<div class="panel panel-default">
				...
				<div class="panel-heading"><b class="titulo">Diseño</b></div>
...
<select name="example" id="example" onChange="showCombo(this.value)" class="form-control">
...
while ($row = $resultado->fetch_array()) {	?>
	<option  value="<?php echo $row ['descripcionDisenio'];?>"
	<?php 
	if( isset($_POST['filtro']) && $_POST['example']==$row['descripcionDisenio']) {
	 	echo "selected"; 
	}?> 
...
<select id="planta" name="planta" class="form-control">
   <option value="0">[Todos]</option> 
   <?php  
$primera = $MySQLiconn->query("SELECT * FROM $confirProd  group by ordenConfi");
while($row1 = $primera->fetch_array()){
	$segunda = $MySQLiconn->query("SELECT orden, sucFK FROM $tablaOrden  where orden='".$row1['ordenConfi']."'");
	while($row2 = $segunda->fetch_array()){ ?>
    	<option value="<?php echo $row2['orden'];?>"><?php echo $row2 ['sucFK']." [" .$row2['orden']."]";?></option>
    	<?php
    }
  }   ?>
</select>
