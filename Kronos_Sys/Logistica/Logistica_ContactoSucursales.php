...
<div class="panel-heading"><b class="titulo">Contactos de sucursales por cliente</b><?php 
...
<select name="example" list="exampleList"  data-live-search="true"  class="selectpicker show-menu-arrow form-control" id="example" onChange="showCombo(this.value)">
  <?php //esta variable es una bandera para la ordenacion de campos en el combo box
$empresa="r";
//Seleccionar todos los datos de la tabla 
$resultado = $MySQLiconn->query("SELECT idsuc, nombresuc, idcliFKS FROM $tablasucursal where bajasuc=1 ORDER BY idsuc, idcliFKS DESC");
...
while ($row = $resultado->fetch_array()) {
  ...
  if(isset($_GET['edit'])){
    if($getROW['sucFK'] == $row['idsuc']){ ?>
      <option value="<?php echo $row['idsuc'];?>" selected> <?php echo $row['nombresuc'];?></option>
      <?php  
    } 
