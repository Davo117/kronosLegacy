...
<div class="panel-heading"><b class="titulo">Sucursales</b><?php 
...
<select name="example" required id="example" onChange="showCombo(this.value)" data-live-search="true"  class="selectpicker show-menu-arrow form-control">
...
while ($row = $resultado->fetch_array()) {
    ...
    if(isset($_GET['edit'])){ 
        if($getROW['idcliFKS'] == $row['ID']){  ?>
            <option value="<?php echo $row['ID'];?>" selected> <?php echo $row['nombrecli'];?></option><?php  
        }
        else{  //Sino manda la lista normalmente:          ?>
            <option value="<?php echo $row ['ID'];?>"><?php echo $row ['nombrecli'];?></option>
            <?php 
        }
    }
    elseif( $row['ID']==$_SESSION['cliente']){ //Si fue seleccionado. en caso de editar el campo manda a llamar el nombre del "fk" sino, muestra el nombre del cliente seleccionado  ?>
        <option value="<?php echo $row['ID'];?>" selected><?php echo $row ['nombrecli'];?>        
        </option> <?php
    }
    else{   //Sino manda la lista normalmente:?>
        <option value="<?php echo $row ['ID'];?>"><?php echo $row ['nombrecli'];?></option>
...
<?php // Creamos un select para traer el nombre del cliente:
//El atributo name sirve para utilizar los option en el crud ?>
<select name="Ciudad" id="Ciudad" required data-live-search="true"  class="selectpicker show-menu-arrow form-control">
...
<div class="col-xs-3">
    <label for="Telefono">Teléfono Suc.</label>
<input type="text" name="Telefono" class="form-control" value="<?php if(isset($_GET['edit'])) echo $getROW['telefonosuc'];  ?>"size="20" placeholder="Número Telefónico" required onkeypress="return numeros(event)">
</div> 
