...
$SQL=$MySQLiconn->query("SELECT * FROM caja where producto='".$productillo."' && cdgEmbarque='' && (baja='2' OR baja='1') ORDER BY id");	

$canti=$MySQLiconn->query("SELECT count(referencia) as cuenta, sum(piezas) as total FROM caja where producto='".$productillo."' && cdgEmbarque='' && baja='2'");
...
<?php 
$canti2=$MySQLiconn->query("SELECT count(referencia) as cuenta, sum(piezas) as total FROM caja where producto='".$productillo."' && cdgEmbarque='".$_GET['cdgEmb']."' && baja='3' ");
$suma2=$canti2->fetch_array();

$SQL=$MySQLiconn->query("SELECT * FROM caja where producto='".$productillo."' && cdgEmbarque='".$_GET['cdgEmb']."'  ORDER BY id"); ?>
...
while ($row=$SQL->fetch_array()) {	?>
	...
	<p class='irrelevance'><td> <a  href="?desactivar=C<?php echo $row["id"].'&cdgEmb='.$_GET['cdgEmb'].'&id='.$_GET['id'].'&emp='.$_GET['emp'].'&pr='.$_GET['pr']; ?>" ><IMG src="funciones/img/deleteOtro.png" title='Modificar' style="margin-bottom: -10px;"></a><?php
	echo "Empaque: <b style='font:bold 100% Sansation;'>"; echo $row['referencia']; echo "</b> &nbsp; Millares: <b style='font:bold 100% Sansation;'>"; echo $row['piezas']; echo "</b></p>
		<p class='irrelevance' style='margin-left:40%;'>Peso: <b style='font:bold 100% Sansation;'>"; echo $row['peso']; echo "</b>
			</td></p>
		</div><br>";
