...
//Posiblemente este documento se elimine 
  include '../../../fpdf/fpdf.php';
  include '../../../Database/db.php';
  

  if ($_GET['numEmbarque'])
  { $_SESSION['numEmbarque'] = $_GET['numEmbarque'];

    $vntsEmbarqueSelect = $MySQLiconn->query("
      SELECT $embarq.numEmbarque,
             $embarq.referencia,
             $impresion.descripcionImpresion,
             $embarq.empaque,
             $embarq.diaEmb,
             $embarq.sucEmbFK,
             $embarq.transpEmb
        FROM $embarq, $impresion
       WHERE $embarq.numEmbarque = '".$_SESSION['numEmbarque']."' AND
             $embarq.producto = $impresion.descripcionImpresion");

    while ($regVntsEmbarque = $vntsEmbarqueSelect->fetch_array()) {
      ...
      $vntsSucursalSelect = $MySQLiconn->query("
        SELECT $tablasucursal.idcliFKS,
               $tablasucursal.nombresuc, 
               $tablasucursal.domiciliosuc,
               $tablasucursal.coloniasuc,
               $tablasucursal.cpsuc,
               $tablasucursal.telefonosuc
          FROM $tablasucursal
          WHERE $tablasucursal.nombresuc = '".$vntsEmbarque_cdgsucursal."'");

      while($regVntsSucursal = $vntsSucursalSelect->fetch_array()){
        $vntsEmbarque_cliente = $regVntsSucursal['idcliFKS'];
...
/*
        $alptEmpaqueSelect = $MySQLiconn->query("
          SELECT alptempaque.cdgempaque,
            ...
            FROM alptempaque,
                 alptempaquep,
                 prodpaquete
           WHERE alptempaque.cdgembarque = '".$_GET['numEmbarque']."' AND
        ...
        while ($regAlptEmpaque = $alptEmpaqueSelect->fetch_array())
          { $item++;
...
/*
      ...
      while ($regAlptEmpaque = $alptEmpaqueSelect->fetch_array())
        { $item++;
          ...
          $alptEmpaqueRSelect = $MySQLiconn->query("
          ...
          $subItem = 0;
          while ($regAlptEmpaqueR = $alptEmpaqueRSelect->fetch_array())
          { $subItem++;
      ...
      { function Header()
        { global $vntsEmbarque_codigodoc;
          ...
          if (file_exists('../../../pictures/logo/labro2.png')==true)
          { $this->Image('../../../pictures/logo/labro2.png',10,0,0,32); }
