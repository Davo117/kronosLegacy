...
if(isset($_POST['save'])){
	...
	$query =$MySQLiconn->query("SELECT prodcliReqFK from $reqProd where bajaReq=1 && ordenReqFK='$idfk' && prodcliReqFK='$idfk2' ");
	if ($query->num_rows>0){
			//echo"<script>alert('Ya existe un Requerimiento con ese producto')</script>";
	}
	else{
...
if(isset($_GET['del']))
{	//Lanzamos la consulta actualizando la baja a 0
	$query = $MySQLiconn->query("SELECT ordenReqFK, prodcliReqFK from $reqProd WHERE idReq='".$_GET['del']."'");
	$row=$query->fetch_array();
	
	$query1 = $MySQLiconn->query("SELECT id from impresiones WHERE codigoCliente='".$row['prodcliReqFK']."'");
	if ($query1->num_rows>0){
		
		while($row1=$query1->fetch_array()){
		$SQL1 =	$MySQLiconn->query("UPDATE confirmarprod SET bajaConfi=3 WHERE ordenConfi='".$row['ordenReqFK']."' && prodConfi='".$row1['id']."'");
		}
	}
