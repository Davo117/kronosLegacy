...
if(isset($_POST['saveA'])){
  ...
  $SQL=$MySQLiconn->query("SELECT referencia, piezas, producto FROM ".$_GET['emp']." where producto='".$product."' && baja='2'"); 
  while ($row=$SQL->fetch_array()) {  
    $cdg='number'.$row['referencia'];
    $chck = $MySQLiconn->real_escape_string($_POST[''.$cdg.'']);

    if ($chck!='--') {
      $updat=$MySQLiconn->query("UPDATE ".$_GET['emp']." set baja='3', cdgEmbarque='".$_GET['cdgEmb']."' where referencia='".$chck."' && producto='".$row['producto']."' ");  
      ...
      $up=$MySQLiconn->query("UPDATE confirmarprod set surtido='".$surtir."' where prodConfi='".$row1['producto']."' && cantidadConfi='".$row1['cantidad']."' && enlaceEmbarque='".$_GET['id']."'"); 
      $contar++;
...
if(isset($_GET['desactivar'])){
  ...
  header("Location: Logistica_ArmadoEmbarque.php?cdgEmb=".$_GET['cdgEmb']."&id=".$_GET['id']."&emp=".$_GET['emp']."&pr=".$_GET['pr']);
} 
