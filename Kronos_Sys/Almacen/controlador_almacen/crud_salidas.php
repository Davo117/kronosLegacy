...
if(isset($_GET['envio']))
{
  ...
  for($i=0;$i<count($arrayP);$i++)
  {
    ...
    $CON2=$mysqli->query("INSERT INTO obelisco.movimientos(idDocumento,tipoDoc,producto,codigoB,costo,fechaMov,horaMov,cantidad,cantidad2,datoextra1) values('$folio','2','".$arrayP[$i]->getCodigo()."','".$arrayP[$i]->getCodigoB()."',".$arrayP[$i]->getPrecio().", '".$arrayP[$i]->getFecha()."','".$arrayP[$i]->getHora()."','".$arrayP[$i]->getCantidad()."','".$arrayP[$i]->getUnidades2()."','".$arrayP[$i]->getOperador()."')");
  }
...
if(isset($_POST['c']) and isset($_POST['p']) and isset($_POST['n']))
{
  ...
  addOut($_POST['c'],$_POST['p'],$_POST['n'],$pr,$un,$codB,$_POST['ope'],$_POST['u2']);
}
function addOut($c,$p,$n,$pr,$un,$codB,$ope,$u2)
{
  include("../modelo_almacen/salida.php");
  include("../../Database/conexionphp.php");
  ...
  //echo number_format($stockreal).'---'.$c;
    if(!empty($c) and !empty($p))
    {
      if ($c <= ceil($stockreal)) {
        ...
        $salidax=new salida();
        $salidax->setNombre($n);
        $salidax->setHora(date("H:i:s", $hora));
        $salidax->setFecha($fecha);
        $salidax->setCantidad($c);
        $salidax->setUnidades2($u2);
        $salidax->setCodigo($p);
