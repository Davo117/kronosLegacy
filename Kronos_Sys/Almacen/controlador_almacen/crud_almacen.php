...
if(isset($_GET['envio']))
{
  if(isset($_GET['proveedor']))
  {
      ...
      for($i = 0; $i < count($arrayP); $i++)
      {
        ...
        $CON2=$mysqli->query("INSERT INTO obelisco.movimientos(idDocumento,tipoDoc,producto,codigoB,costo,fechaMov, horaMov,cantidad,cantidad2,lote) values('".$folio."','".'1'."','".$arrayP[$i]->getCodigo()."','".$arrayP[$i]->getCodigoB()."',".$arrayP[$i]->getPrecio().", '".$arrayP[$i]->getFecha()."', '".$arrayP[$i]->getHora()."' ,'".$arrayP[$i]->getCantidad()."','".$arrayP[$i]->getUnidades2()."','".$arrayP[$i]->getLote()."')");
      }
...
if(isset($_POST['c']) and isset($_POST['p']) and isset($_POST['n']) and isset($_POST['h']) and isset($_POST['fechaIn']))
{
  ...
  addIn($_POST['c'],$_POST['p'],$_POST['n'],$_POST['h'],$pr,$un,$codB,$lote,$_POST['fechaIn'],$_POST['u2']);
}

function addIn($c,$p,$n,$h,$pr,$un,$codB,$lote,$fecha,$u2)
{
  include "../modelo_almacen/entrada.php";
  session_start();
...
function getunidades($b) {
  ...
  if(!empty($fet['producto'])){
    ...
    $mySQL=$mysqli->query("SELECT u.nombreUnidad as unidad,(SELECT nombreUnidad from saturno.unidades where idUnidad=p.unidad2) as unidad2, u.identificadorUnidad as alias, p.hascode, p.hasprice FROM saturno.unidades u inner join obelisco.productosCK p on p.unidad = u.idUnidad WHERE p.producto='".$b."'");
        ...
        <center style="border-style:solid;border-radius:5px;border-color:#E5E5E5;border-width:2px;padding:5px">
          ...
          <form id="setProducto" method="POST" style="">
            <div style="width:25%;">
              ...
              <input type="number" placeholder="<?php echo $rut['unidad2']?>" name="txtcantidad2" id="cantidad2" value="" class="form-control">
  ...
  <h4>Lista de productos</h4>
    <div class="container-fluid">
      ...
      <table class="table table-hover">
          <?php
          $detalleEntrada=$mysqli->query("SELECT m.*,(SELECT identificadorUnidad from saturno.unidades where idUnidad=p.unidad) as unidad1,(SELECT identificadorUnidad from saturno.unidades where idUnidad=p.unidad2) as unidad2 FROM obelisco.movimientos m inner join obelisco.productosCK p on p.producto=m.producto WHERE m.idDocumento = ".$id." AND m.tipoDoc = 1");
          ?>
          <thead>
          <tr>
            <th>Código</th>
          ...
          <tbody>
            ...
            while($row=$detalleEntrada->fetch_array())
            {
              ...
              <tr>              
                ...
                <td><?php echo $row['cantidad'].$row['unidad1'].'<br>'.$row['cantidad2'].$row['unidad2'];?></td>
