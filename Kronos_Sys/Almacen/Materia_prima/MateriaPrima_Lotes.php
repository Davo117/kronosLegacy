...
if(isset($_POST['comboBloquesillos']))
{
  $bloqueDescripcion=$_POST['comboBloquesillos'];
}
else if(isset($_GET['bloqueson']))
{
  $bloqueDescripcion=$_GET['bloqueson'];
}
else
{
...
<div class="panel panel-default">
 ...
 <label for="comboBloquesillos">Bloque</label>
  <select onChange="document.formulary.submit()"  class="form-control" name="comboBloquesillos" id="comboBloquesillos">
    <?php
...
while ($row = $resultado->fetch_array()) {
	...
	if( $row ['idSustrato']==$bloqueDescripcion)
{
	?>
	<option value="<?php echo $row ['idSustrato'];?>" selected><?php echo $row['descripcionSustrato'];?></option>
	<?php
}
	else{
		?>
...
//
$MySQLiconn->query("UPDATE tblotes set shower=1");

$resaltado= $MySQLiconn->query("SELECT tarima from tblotes where  bloque='".$bloqueDescripcion."'");
$rowes = $resaltado->fetch_array();
$rowe_cnt=$resaltado->num_rows;
printf("<p id='mostrar'>Se muestran: ".$rowe_cnt." Registros Activos</p>");
$resaltado->close(); 
//
$resultado = $MySQLiconn->query("SELECT count(*),tarima from tblotes where  bloque='".$bloqueDescripcion."' group by tarima");
...
if($row_cnt<=1){
	$resultado = $MySQLiconn->query("SELECT tarima from tblotes where  bloque='".$bloqueDescripcion."' order by tarima asc");
	...
	$resultado = $MySQLiconn->query("SELECT tarima from tblotes where  bloque='".$bloqueDescripcion."' and tarima !='".$_SESSION['tarima']."' and shower=1 order by tarima asc");

	$cant= mysqli_fetch_array($resultado);
	$_SESSION['tarima']=$cant[0];
	$MySQLiconn->query("UPDATE tblotes set shower=0 where tarima='$cant[0]'");
