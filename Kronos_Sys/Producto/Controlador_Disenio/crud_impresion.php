	...
	if(isset($_POST['save']))
	{
		...
		$code=$_POST['codigoImpresion'];
		$SQL=$MySQLiconn->query("SELECT codigoImpresion from impresiones where codigoImpresion='".$code."'");
		$result=$SQL->fetch_array();
		if(empty($result['codigoImpresion']))
		{
	$productoscliente=$MySQLiconn->real_escape_string($_POST['comboPCliente']);
		...
		$ref=$MySQLiconn->query("SELECT refil FROM producto WHERE id='$tipo'");
		$rel=$ref->fetch_array(); 

		$your=$MySQLiconn->query("SELECT anchura FROM sustrato where idSustrato='$sustratos'");
		$my=$your->fetch_array();
		...
		if($_POST['alturaEtiqueta']>0 && $_POST['anchoPelicula']>0)
		{
			if(isset($_POST['espacioFusion']))
			{
				...
				else if(($my['anchura']+($my['anchura']*0.3))>=(($_POST['anchoPelicula']+$_POST['espacioFusion'])+$_POST['espacioregistro']))
				{
					$bigger=1;
				}
				else
			...
			if($bigger==1)
			{
				...
				if($valPeli==1)
				{
					if($allowref==1)
					{
						...
						if($isEntire==1)
						{
	$Sic =$MySQLiconn->query("INSERT INTO impresiones(descripcionDisenio,codigoCliente,nombreBanda,sustrato,holograma,haslote) VALUES('$tipo','$productoscliente','$bspp','$sustratos','$holog','$lote')");//Hace un insert,creando un nuevo registro


	$SQL = $MySQLiconn->query("SELECT nombreparametro FROM juegoparametros where identificadorJuego=(SELECT juegoparametros from tipoproducto where id=(SELECT tipo from producto where id='$tipo'))");
	...
	else
	{
		...
		else
		{
			echo "<div class='alert alert-warning alert-dismissible fade in'>
			<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
			<strong>Datos duplicados</strong> , Ese código ya existe.
			</div>";
		}
	...
	if(isset($_POST['update']))
	{
		...
		$your=$MySQLiconn->query("SELECT anchura FROM sustrato where idSustrato='".$_POST['comboSustratos']."'");
		$my=$your->fetch_array();
		if(isset(($_POST['espacioregistro'])))
		{
	...
	$Sic =$MySQLiconn->query("UPDATE impresiones set codigoCliente='$productoscliente',nombreBanda='$bspp',sustrato='$sustratos',holograma='$hologram',haslote='$lote' where codigoImpresion='$codigoImpresion'");
	...
	if(isset($_GET['cil']))
	{
		...
		header("Location: Producto_JuegosCilindros.php?cil=".$_GET['cil']."");
	...
	if(isset($_GET['delfin']))
	{
		...
		$MySQLiconn->query("DELETE from juegoscilindros where descripcionImpresion=".$_GET['delfin']."");
		$MySQLiconn->query("DELETE from juegoscireles where producto=".$_GET['delfin']."");
		//$MySQLiconn->query("DELETE from suaje where descripcionImpresion=".$_GET['delfin']."");
		$MySQLiconn->query("DELETE from pantonepcapa where codigoImpresion=(select codigoImpresion from impresiones where id=".$_GET['delfin'].")");
	...
	if(isset($_GET['imp'])){
		...
		header("Location: Producto_Cireles.php?producto=".$_GET['imp']);	
	} 
