...
if(isset($_POST['save'])){
	...
	$id = trim($_POST['identificadorS']);
	$provee = trim($_POST['proveedor']);
	$cdg = trim($_POST['cdg']);
	...
	$tipoherr=$_POST['tipoherr'];

	//Realizamos la consulta

	$cat=$MySQLiconn->query("
	SELECT s.anchura,i.anchoPelicula,i.espacioregistro,p.cilindros,p.cireles FROM sustrato s INNER JOIN impresiones i
    ON  s.idSustrato=i.sustrato 
	...
	$pieza=($tabla-$dim['espacioregistro'])/$dim['anchoPelicula'];
	$consulta=$MySQLiconn->query("SELECT anchoPelicula, alturaEtiqueta from impresiones where id='$prod' and baja='1'");
	...
	{
		//Si la altura real no coincide con la del herramental no deja guardar
		if($dim['cilindros']==1)
		{
			$skin=$MySQLiconn->query("SELECT alturaReal FROM juegoscilindros WHERE descripcionImpresion='".$prod."' AND baja=1");
			$still=$skin->fetch_array();
			if($alturaReal<($still['alturaReal']*1.5) && $alturaReal>($still['alturaReal']*0.5))
			{
				$herramental=1;
			}
		}
		if($dim['cireles']==1)
		{
			...
			$skin=$MySQLiconn->query("SELECT alturaReal FROM juegoscireles WHERE producto='".$prod."' AND baja=1 ");
			$still=$skin->fetch_array();
			if($alturaReal<($still['alturaReal']*1.5) && $alturaReal>($still['alturaReal']*0.5))
			{
				$herramental=1;
...
if($rowA['alturaEtiqueta']+($rowA['alturaEtiqueta']*1.5)>$alturaReal && $rowA['alturaEtiqueta']-($rowA['alturaEtiqueta']*1.5)<$alturaReal)
{

	if($herramental==1)
	{
		$SQL =$MySQLiconn->query("INSERT INTO suaje (identificadorSuaje, proveedor, codigo, alturaImpresion, anchuraImpresion, piezas, alturaReal, corteSeguridad, descripcionImpresion, observaciones, reguardo, proceso, tipoherr) VALUES ('$id', '$provee', '$cdg', '".$rowA['alturaEtiqueta']."', '".$rowA['anchoPelicula']."', '$pieza', '$alturaReal', '$corte', '$prod', '$observacion', '$res', '$process','$tipoherr')");
	...
	{
		echo"<script>alert('La altura real no coincide con la del herramental utilizado')</script>";
	}
}
else
{
