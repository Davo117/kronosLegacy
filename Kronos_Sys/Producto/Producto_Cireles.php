...
<select id="combillo" name="producto" required onChange="document.formulary.submit()" class="form-control">
  
    <option value="0" <?php if(empty($producto)){ echo "selected";}?>>--</option>
    <?php
    $consulta=$MySQLiconn->query("SELECT descripcion,ID FROM producto where baja=1 && cireles=1");
while ($row1=$consulta->fetch_array()) {
   
	$segunda=$MySQLiconn->query("SELECT id,descripcionImpresion, codigoImpresion, descripcionDisenio FROM impresiones where baja='1' and descripcionDisenio='".$row1['ID']."'");
    	...
    	if(isset($_GET['edit'])){
    		if ($row2['id']==$getROW['producto']) { ?>
    			...
    			<?php 
      			...
      			<option value="<?php echo $row2['id'];?>"><?php echo $row2['descripcionImpresion'];?></option>
	      		<?php 
    		}
    	}
    	elseif($row2['id']==$producto) { ?>
	      <option value="<?php echo $row2 ['id'];?>" selected><?php echo $row2['descripcionImpresion'];?>  </option>
    		<?php 
    	}
...
<input maxlength="10" type="text" class="form-control" required name="num_dientes" id="num_dientes" value="<?php if(isset($_GET['edit'])){echo $getROW['num_dientes']; if($getROW['prod']==1){echo '"readonly="true';}}?>" onkeypress="return numeros(event)"  placeholder="#">
</div>
<?php
if(isset($_GET['edit']))
{?>
<div class="col-xs-3">
<label for="repeticiones">Repeticiones a la tabla:</label>
...
<?php  
	if(isset($_GET['edit'])){	?>
		...
		echo "<p>Placa ".$i;?>
			...
			<p style=" border-radius:2px;border-color:black; font-size:13px;color:black; padding:5px;margin:5px;width:20%;height:5%; background-color:<?php echo '#'.$tintas['codigoHTML']; ?>"><?php  echo $tintas['descripcionPantone'];?></p> </p>
		<?php $i++;
...
<div class="table-responsive">
                                <table border='0' cellpadding='15'  class="table table-hover">
                                    <?php
                                    include ("Controlador_Disenio/showCireles.php");
                                    ?>
                                </table>
                            </div>
...
if(isset($_POST['checkTodos'])){?>
    ...
    <div class="table-responsive">
                                <table border='0' cellpadding='15'  class="table table-hover">
                                    <?php
                                    include ("Controlador_Disenio/showCireles_bajas.php");
                                    ?>
                                </table>
                            </div>   
    <?php
