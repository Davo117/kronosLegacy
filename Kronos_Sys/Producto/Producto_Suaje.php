...
if (isset($_SESSION['logan']) && $_SESSION['logan'] == true) { 
	...
	<form method="POST" name="formulary" id="formulary" role="form">
            <div class="panel panel-default">
                ...
                <div class="panel-heading"><b class="titulo">Suaje/Máster</b>
			<?php 
...
<select id="combillo" class="form-control" name="producto" required onChange="showCombo(this.value)">
    ...
    <option value>--</option>
    <?php
    $consulta=$MySQLiconn->query("SELECT ID FROM producto where baja=1 && suaje=1");
while ($row1=$consulta->fetch_array()) {
   
	$segunda=$MySQLiconn->query("SELECT descripcionImpresion,id, codigoImpresion, descripcionDisenio FROM impresiones where baja='1' and descripcionDisenio='".$row1['ID']."'");
  	$empresa='ssss';
  	...
  	while($row2=$segunda->fetch_array()){ //mientras se tengan registros:
    	...
    	if(isset($_GET['edit'])){
    		if ($row2['id']==$getROW['id']) { ?>
	    		<option value="<?php echo $getROW ['id'];?>" selected><?php echo $row2['descripcionImpresion'];?>  </option>
    			<?php 
    		}
...
<div class="col-xs-1">
    <label>Tipo</label>
    <select class="form-control" name="tipoherr">
        <option value="1" selected="">Suaje</option>
        <option value="2">Máster</option>
    </select>
</div>
<div class="col-xs-2">
<label for="identificadorS">Identificador:</label>
<input type="text" class="form-control" required name="identificadorS" id="identificadorS" value="<?php if(isset($_GET['edit'])) echo $getROW['identificadorSuaje'];?>"  placeholder="Nombre Suaje">
</div>
<div class="col-xs-3">
...
<select name="proceso" class="form-control" required id="proceso">
    ...
    <option value="18">Impresión-flexográfica</option>
    <option value="14">Troquelado</option>
    <option value="15">Suajado</option>
</select>
</div> 
</div>
<?php  
