					...
					if(isset($_POST['checkTodos']))
					{
						...
						<div class="panel-body">
							...
							<div class="col-xs-3">
								...
								<input id="descripcion" type="text" class="form-control" name="descripcion" autocomplete="off" value="<?php if(isset($_GET['edit'])) echo $getROW['descripcion'].'" readonly="false ';  ?>" size="20" placeholder="Descripción o nombre" required>
								<div id="resultado">
								</div>
							</div>
							<div class="col-xs-3">
							...
							<div class="col-xs-3">
								...
								<SELECT required name="tipocombo" class="form-control" id="tipocombo" title="Seleccione el tipo de producto según las carácteristicas" method="post" > 
									<?php
									$resultado=$MySQLiconn->query("SELECT alias as tipo,presentacion,id from tipoproducto where baja=1");
									?>
									<option value="">--</option>
									<?php
									while($row=$resultado->fetch_array())
									{
										...
										{	
											...
											if($getROW['tipo']==$row['id'])
											{
												
												
												?>
												<OPTION selected VALUE="<?php echo $row['id']; ?>"  ><?php echo $row['tipo'].' ['.$presentacion.']'; ?></OPTION>
												<?php
											}
											else
											{
												?>
												<OPTION VALUE="<?php echo $row['id']; ?>" ><?php echo $row['tipo'].' ['.$presentacion.']'; ?></OPTION>
												<?php
											}
										}
									}
								...
								{
									?>
									<button class="btn btn-default" style="float:right;" <?php if(substr($privilegios,0,2)=='rwx'){ echo "disabled";}?> type="submit" name="save">Guardar</button>
									<?php
								}?>
							</form>
