...
* {margin:0; padding:0;}
  ...
  box-shadow: 0 0 0px ;}
	...
	#contenedor {background-color: #CDCDCD; margin:10px auto; padding:10px 40px; width:60%; border-radius:4%;}
	
	#calendario {width:100%; height: 350px; font-style: Sansation;}
	
	#calendario div.tabla {position:relative; width:100%; height:100%;  padding-bottom:25%;}
	...
	#calendario table tbody td {border:1px solid #bbb; border-width:0 1px 1px 0 ; text-align:center; font-size: 18px; width: 50px; height: 50px;}
	...
	#calendario ul li.siguiente {right:0; cursor: pointer;}
...
<body>
	<center>
<h2 style="color:#5D6770; font-family: Sansation; "> Calendario de Entregas </h2>
<div id="contenedor">
...
function calendario ($year,$mes,$finDeSemana=1,$mostrarDiasNulos=1,$nivelH=2) {
	...
	print( '<li class="anterior" ><a href="?dia=1&amp;mes='.$mesAnterior.'&amp;ano='.$yearMesAnterior.'" ><img style="cursor:pointer" src="funciones/img/go-previous.png" alt="Mes anterior ('.$meses[($mesAnterior-1)].')" height="25" width="25"></a></li>');
	print( '<li class="siguiente"><a href="?dia=1&amp;mes='.$mesSiguiente.'&amp;ano='.$yearMesSiguiente.'"><img src="funciones/img/go-next.png" alt="Mes anterior ('.$meses[($mesAnterior-1)].')" height="25" width="25" style="cursor: hand;" ></a></li>');
	print( '</ul>');
	...
	$mesLista = $mes-1;
	print( '<br><caption class="holo">'.$meses[$mesLista].' del '.$year.'</caption>');
	...
	while ($dm<=$diasEnMes) {
	 ...
	 $SQL =$MySQLiconn->query("SELECT embarqueConfi FROM $confirProd where enlaceEmbarque=0 && embarqueConfi='".$fechaEmb."'");
 		//la variable $dm contiene el numero del dia
	 	$filtro='<'.$tagEnlace.' class="enlace" href="'.$atribEnlace.'">'.$dm.'</'.$tagEnlace.'>';
 		$rowers=$SQL->fetch_array();
 		if ($rowers['embarqueConfi']==$fechaEmb) {
 			$filtro='<a target="_blank" style=" background-color: transparent; color: transparent;" href="funciones/pdf/EntregasPDF.php?dsd='.$fechEntrega.'&hst='.$fechEntrega.'&suc=--&prod=--" onclick="return confirm("Gracias por hacer clic"); " ><IMG  width="30" src="../pictures/icono-PDF.png" title="Descargar" /></a>';

 			
 		}

 		//*************************** FIN CODIGO **********************************
		print( '<td id="'.$idFecha.'" class="'.calendarioClaseDia($ds).$claseActual.'"><'.$tagDia.' class="dia">'.$filtro.'</'.$tagDia.'></td>');
		$dm++;$x++;$ds++;
	}
...
<hr>
<div  id="formularius" class="col-sm-12 col-xs-12 col-lg-12">
<form>
...
<div class="col-ms-3 col-xs-3 col-lg-3">
...
<div class="col-ms-3 col-xs-3 col-lg-3">
<label>Desde:</label>
<input type="date" class="form-control" value="<?php  echo $concat; ?>" name="fechaDesde">
</div>

<div class="col-ms-3 col-xs-3 col-lg-3">
<label>Hasta:</label>
<input type="date" class="form-control" value="<?php  echo $concat;?>" name="fechaHasta">
</div>
<div class="col-ms-3 col-xs-3 col-lg-3">
<button type="submit" name="download" class="btn btn-info">Descargar</button>
</div>
<br>
