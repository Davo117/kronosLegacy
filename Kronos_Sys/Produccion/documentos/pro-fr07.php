...
$pdtoImpresionSelect = $MySQLiconn->query("
  ...
  $pdf->Cell(($numLotes*10),5,'noop',1,0,'C');
  $pdf->Ln();

  $pdf->SetX(30);

  for ($idLote = 1; $idLote <= $numLotes; $idLote++)
  { $pdf->Cell(10,5,$oplist_noop[$idLote],1,0,'C'); }
  
  $pdf->Ln();

  $pdf->SetFont('Arial','',8);
  $pdf->SetX(21);
  $pdf->Cell(9,5,'Hora',1,0,'C');
  for ($idLote = 1; $idLote <= $numLotes; $idLote++)
  { $pdf->Cell(10,5,':',1,0,'C'); }
  ...
  { $pdf->SetFont('Arial','B',11);
    ...
    $pdf->SetFont('Arial','',6);
    $pdf->Cell(15,5,'Viscosidad/pH',1,0,'R');
    ...
    $pdf->SetX(15);
    $pdf->Cell(15,5,'Temperatura',1,0,'R');
  ...
  $pdf->Cell(20,5,utf8_decode('Velocidad'),1,0,'R');  
    ...
    $pdf->Cell(20,5,utf8_decode('Tensión holograma'),1,0,'R');  
    for ($idLote = 1; $idLote <= $numLotes; $idLote++)
    { $pdf->Cell(10,5,'',1,0,'C'); }
    $pdf->Ln();
