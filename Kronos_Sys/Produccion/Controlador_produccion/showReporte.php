...
$tipo=$_GET['tipo'];
if(empty($row['noLote'])|| empty($_GET['e']) and !empty($_GET['c']))
{
	if($tipo!="1")
	{
$result=$MySQLiconn->query("SELECT concat(lotes.tarima,' | ',lotes.numeroLote) as loteDisponible,lotes.peso,lotes.longitud,lotes.referenciaLote,(select anchura from sustrato where idSustrato=(select sustrato from impresiones where id='".$_GET['c']."')) as mm from lotes
 inner join sustrato on sustrato.idSustrato=lotes.idbloque 
where sustrato.idSustrato=(select sustrato from impresiones where id='".$_GET['c']."') and lotes.estado=2 and lotes.baja=1 and sustrato.baja=1");
	}
	else
	{

		$result=$MySQLiconn->query("SELECT concat(lotes.tarima,'|',lotes.numeroLote) as loteDisponible,lotes.peso,lotes.longitud,(SELECT repeticiones from bandaspp where idBSPP='".$_GET['c']."') as anchuraBS,
lotes.referenciaLote,(SELECT anchura from sustrato where idSustrato=(SELECT
sustrato from bandaspp where idBSPP='".$_GET['c']."' limit 1)) as mm from lotes
inner join sustrato on sustrato.idSustrato=lotes.idbloque where sustrato.idSustrato=(SELECT sustrato from bandaspp where idBSPP='".$_GET['c']."')
and lotes.estado=2 and lotes.baja=1 and sustrato.baja=1");
	}


?>
<p id="leyendalotes" style="text-align:justify;width:400px;margin-right:310px;">Lotes disponibles (<?php echo $result->num_rows;?>)</p><br>

<?php
while($row=$result->fetch_array())
{

	$Sick=$MySQLiconn->query("SELECT anchoPelicula,alturaEtiqueta from impresiones where id='$descripcionImpresion' and baja=1");
	...
	{
		$unidades=$row['anchuraBS']*$row['longitud'];
	}
	else
	{
		$unidades='?';
...
else
{
	...
	$MySQLiconn->query("INSERT INTO lotestemporales (noLote, longitud, unidades, anchura, peso, referencia,tipo) VALUES ('".$row['loteDisponible']."', '".$row['longitud']."', '$unidades', '".$row['mm']."', '".$row['peso']."', '".$row['referenciaLote']."','$tipo')");
}
?>
</div>
<?php
