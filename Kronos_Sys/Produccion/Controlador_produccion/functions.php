...
sustrato from bandaspp where idBSPP=bandaspp.IdBSPP limit 1)) as mm,bandaseguridad.anchura from `tbpro".$proceso."` inner join bandaspp on bandaspp.idBSPP=`tbpro".$proceso."`.producto inner join bandaseguridad on bandaspp.identificadorBS=bandaseguridad.IDBanda where `tbpro".$proceso."`.producto='$producto' and `tbpro".$proceso."`.noop like '$noop' and `tbpro".$proceso."`.tipo='$tipo'");//Trae todos los parametros de ese registro para calcular las unidades
	...
	$obj=$SN->fetch_assoc();
		if($longitud==true)//si es un parametro con longitud,se hace una serie de calculos dependiendo del proceso
		{
			...
			{
				$unidades=(($obj['repAlPaso']/$obj['alturaReal'])*$obj['longitud']);
				//echo $obj['repAlPaso']."---".$obj['alturaReal']."---".$obj['longitud'];
			}
			...
			else if($proceso=="suajado")
			{
				...
				else if($posRef<$act)
				{
					...
					if($obj['refParcial']>1)
					{
						...
						{//Se modificó la FÓRUMULA PARA QUE MANDARA POR DOS
	...
	function sendDivisions($tipo,$procesoSiguiente,$procesoActual,$producto,$codigo,$noop,$MySQLiconn)
	{
		...
		if($tipo!="1")
		{
			...
			else if($procesoSiguiente=="corte")
			{
				...
				$bit=$SQL->fetch_assoc();
				$divisiones =ceil($bit['unidades']/$bit['millaresPorPaquete']);//anteriormente en lugar de round estaba number_format
			}
			...
			else if($procesoSiguiente=="sliteo")
			{
				...
				if(!$SQL)
				{
					...
					/*
					Se modificó porque en sliteo se tenía que mandar varias divisiones y se corta transversal y horizontalmente
					if($rik['refParcial']==1)
					{
						$arrayDatos=parsearCodigo($codigo,$MySQLiconn);
					$arr=explode('|',$arrayDatos);
					...
					//Anteriormente redondeada hacia abajo
					$divisiones=ceil(($rik['unidades']/$rik['millaresPorRollo']));
					/*}*/
	...
	if($idtipo!="1") //Si no es banda de seguridad
	{
			...
			else if($processName=="corte")
			{
				...
				if ($consul->num_rows > 0)
				{
					...
					$mpp= $regCodePaquete["millaresPorPaquete"]*1000;
						$nCodigos=ceil(($unidades/$mpp));//Anteriormente estaba ceil en lugar de round
	 ...
	 // Código de barras
							...
							//Esta variable es el número de hojas en el documento de etiq, son dos etiquetas por hoja
	    					$nPaquetes=ceil((($unidades/$mpp)/2));//Antes estaba number_format
	    					$nPac=ceil($unidades/$mpp);//Ants estaba number_format
	    					...
	    					$label="<script>window.location='$stringfile?codePaquete_cdgpaquete=$codeList1&codePaquete_paquete=$codeList2&idtipo=$idtipo&nPaquetes=$nPaquetes&codigo=$codigo&conte=$mpp&lote=$lotesin'</script>";
	    				}
	    			...
	    			else if($processName=="sliteo") 
	    			{
	    				...
	    				if ($consul->num_rows > 0)
	    				{
	    					//En sliteo index inicia en 1 para poder obtener los códigos como en corte
	    					$index=1;
	    					while ($regPackingList = $consul->fetch_assoc())
	    					{
	    						$SIC=$MySQLiconn->query("SELECT amplitud from `tbpro".$processName."` where noop  like '".$regPackingList["noop"]."' and tipo='".$idtipo."'");
