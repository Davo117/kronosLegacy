			...
			$msj=base64_encode($msj);*/
				...
				if(!empty($_GET['cil']))
				{
					...
					if(!empty($_GET['cir']))
					{
						...
						{
							$SQ=$MySQLiconn->query("
								SELECT  l.juegoLote,l.unidades,l.referenciaLote,l.noop,p.cantLotes,l.tipo,(SELECT codigo from tbcodigosbarras where lote=l.referenciaLote and noProceso=0 ) as codigo from tblotes l left join tbproduccion p on p.juegoLotes=l.juegoLote where l.idLote='".$_GET['return']."'");
							...
							$fstProcess=$MySQLiconn->query("SELECT j.descripcionProceso,p.nombreparametro FROM juegoprocesos j inner join tipoproducto t on t.juegoProcesos=j.identificadorJuego inner join procesos pro on pro.descripcionProceso=j.descripcionProceso inner join juegoparametros p on p.identificadorJuego=pro.packParametros WHERE t.id='".$rem['tipo']."' and j.numeroProceso=1 and p.numParametro='C'");

							$asocFst=$fstProcess->fetch_assoc();
							$MySQLiconn->query("DELETE FROM `tbpro".$asocFst['descripcionProceso']."` WHERE noop like '".$rem['noop']."%' and tipo='".$rem['tipo']."' and ".$asocFst['nombreparametro']."='".$rem['codigo']."'");
