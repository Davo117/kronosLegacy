...
while ($confirmado=$confirmar->fetch_array()){
    ...
    while($segunda=$segundo->fetch_array()){
      ...
      if ($sucursal==$confirmado['ordenConfi']) {
        ...
        //if producto==--
          ...
          $pdf->Cell(17,4,$confirmado['empaqueConfi'],1,0,'L',true);
          $pdf->Cell(18,4,number_format($confirmado['cantidadConfi'],3),1,1,'R',true); 
          if(array_search($row2['codigoImpresion'],$arrayTotales[$row2['codigoImpresion']]))
          {
            if(!empty($arrayTotales[$row2['codigoImpresion']]['cantidadCaja']))
            {
              $arrayTotales[$row2['codigoImpresion']]['cantidadCaja']=$confirmado['cantidadConfi']+$arrayTotales[$row2['codigoImpresion']]['cantidadCaja'];
            }
            else if(!empty($arrayTotales[$row2['codigoImpresion']]['cantidadRollo']))  
            {
              $arrayTotales[$row2['codigoImpresion']]['cantidadRollo']=$confirmado['cantidadConfi']+$arrayTotales[$row2['codigoImpresion']]['cantidadRollo'];
            } 
            else
            {
              if($confirmado['empaqueConfi']=="caja")
              {
                $arrayTotales[$row2['codigoImpresion']]['cantidadCaja']+=$confirmado['cantidadConfi'];
              }
              else if($confirmado['empaqueConfi']=="rollo")
              {
                $arrayTotales[$row2['codigoImpresion']]['cantidadRollo']+=$confirmado['cantidadConfi'];
              }
            }          
          } 
          else
          {
            $arrayTotales[$row2['codigoImpresion']]['nombre']=$row2['codigoImpresion'];
            if($confirmado['empaqueConfi']=="caja")
            {
              $arrayTotales[$row2['codigoImpresion']]['cantidadCaja']=$confirmado['cantidadConfi'];
            }
            ...
            $arrayTotales[$row2['codigoImpresion']]['empaque']=$row2['empaqueConfi'];
            $arrayTotales[$row2['codigoImpresion']]['producto']=$row2['descripcionImpresion'];
          } 
          if ($confirmado['empaqueConfi']=='caja') {
            $cuentaC+=$confirmado['cantidadConfi'];
          }
  ...
  elseif ($_GET['prod']=='--' && $_GET['suc']=='--') {
    ...
    $pdf->Cell(18,4,number_format($confirmado['cantidadConfi'],3),1,1,'R',true); 
          ...
          if(array_search($producto['codigoImpresion'],$arrayTotales[$producto['codigoImpresion']]))
          {
            if(!empty($arrayTotales[$producto['codigoImpresion']]['cantidadCaja']))
            {
              $arrayTotales[$producto['codigoImpresion']]['cantidadCaja']=$confirmado['cantidadConfi']+$arrayTotales[$producto['codigoImpresion']]['cantidadCaja'];
            }
            else if(!empty($arrayTotales[$producto['codigoImpresion']]['cantidadRollo']))  
            {
              $arrayTotales[$producto['codigoImpresion']]['cantidadRollo']=$confirmado['cantidadConfi']+$arrayTotales[$producto['codigoImpresion']]['cantidadRollo'];
            }
            else
            {
              if($confirmado['empaqueConfi']=="caja")
              {
                $arrayTotales[$producto['codigoImpresion']]['cantidadCaja']+=$confirmado['cantidadConfi'];
              }
              else if($confirmado['empaqueConfi']=="rollo")
              {
                $arrayTotales[$producto['codigoImpresion']]['cantidadRollo']+=$confirmado['cantidadConfi'];
              }
            }          
          } 
          else
          {
            $arrayTotales[$producto['codigoImpresion']]['nombre']=$producto['codigoImpresion'];
            if($confirmado['empaqueConfi']=="caja")
            {
              $arrayTotales[$producto['codigoImpresion']]['cantidadCaja']=$confirmado['cantidadConfi'];
            }
            ...
            $arrayTotales[$producto['codigoImpresion']]['empaque']=$confirmado['empaqueConfi'];
            $arrayTotales[$producto['codigoImpresion']]['producto']=$producto['descripcionImpresion'];
          } 
...
$pdf->Cell(4,4,'',0,0,'C');
$pdf->SetFont('arial','B',8);
$pdf->Cell(100,4,'Totales (Caja)',1,1,'L',true);
$pdf->SetFillColor(248,215,205);
$pdf->SetFont('arial','',8);
$pdf->setX(14);
foreach ($arrayTotales as &$valor) {
  if($valor['empaque']=="caja")
  {
    $pdf->SetFont('arial','',8);
    $pdf->Cell(80,4,$valor['producto'],1,0,'L',true); 
    $pdf->SetFont('arial','B',8); 
    $pdf->Cell(20,4,number_format($valor['cantidadCaja'],3),1,1,'R',true); 
    $pdf->setX(14);    
  }
}
$pdf->setX(94);
$pdf->Cell(20,4,number_format($cuentaC,3),1,1,'R',true);

$pdf->Ln(5);
$pdf->SetFillColor(237,125,49);
$pdf->Cell(4,4,'',0,0,'C');
$pdf->SetFont('arial','B',8);
$pdf->Cell(100,4,'Totales (Rollo)',1,1,'L',true);
$pdf->SetFillColor(248,215,205);
$pdf->SetFont('arial','',8);
$pdf->setX(14);
foreach ($arrayTotales as &$valor) {
  if($valor['empaque']=="rollo")
  {
    $pdf->SetFont('arial','',8);
    $pdf->Cell(80,4,$valor['producto'],1,0,'L',true); 
    $pdf->SetFont('arial','B',8); 
    $pdf->Cell(20,4,number_format($valor['cantidadRollo'],3),1,1,'R',true); 
    $pdf->setX(14);    
  }
}
$pdf->setX(94);
$pdf->Cell(20,4,number_format($cuentaR,3),1,1,'R',true);
