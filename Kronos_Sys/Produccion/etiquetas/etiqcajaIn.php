...
$consul=$MySQLiconn->query("SELECT e.*, i.descripcionImpresion as prod,i.logproveedor,date(e.fechamov) as fecha,DATE(DATE_ADD(e.fechamov, INTERVAL 6 MONTH)) as caduca from caja e inner join impresiones i
    on e.producto=i.id  where e.codigo='$numero'");//Esta consulta tiene longitud,carece de dinamicidad
...
$contador=0;//Se inicializa un contador
  ...
  { 
    ...
    $pdf->Cell(32,12,''.$regPackingList->referencia,0,1,'R'); 
 ...
 $pdf->Ln(1);
    ...
    if($regPackingList->peso>0)
    {
      ...
      $pdf->SetFont('Arial','',10);
      $pdf->Cell(4,6,'',0,0,'L'); 
      $pdf->Cell(35,6,'Peso',0,0,'L');
      $pdf->SetFont('Arial','B',10);  
      $pdf->Cell(15,6,$regPackingList->peso." kgs",0,1,'R');
    }
