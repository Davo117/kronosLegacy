...
$proceso=explode("|", $numero);
    $producto=$proceso[1];
    $proceso=strtolower($proceso[0]);


       $MySQLiconn->query("SET @p0='".$producto."'");
$SQL=$MySQLiconn->query("call getTipo(@p0)");
       $cso=$SQL->fetch_array();
       $tipo=$cso['id'];//El id del proceso actual
       $MySQLiconn->next_result();

if($tipo!="1"){
  $SQL=$MySQLiconn->query("SELECT numeroProceso from juegoprocesos where identificadorJuego=(select juegoProcesos from tipoproducto where  id=(select tipo from producto where 
  ...
  $SQL=$MySQLiconn->query("SELECT numeroProceso from juegoprocesos where identificadorJuego=(select juegoProcesos from tipoproducto where id=1 and descripcionProceso='".$proceso."')");
  $roa=$SQL->fetch_array();
  $noProces=$roa['numeroProceso'];
}
...
if($tipo!="1"){
  ...
  $procesoActual=strtolower($consRo['actual']);//la descripcion del proceso que se esta registrando
  $MySQLiconn->next_result();

  $consul=$MySQLiconn->query("SELECT `tbpro".$procesoActual."`.*,impresiones.id as newid,(select descripcion from producto where ID=impresiones.descripcionDisenio) as descripcionDisenio,impresiones.descripcionImpresion,impresiones.anchoPelicula,impresiones.anchoEtiqueta,impresiones.espacioFusion,impresiones.alturaEtiqueta,impresiones.refParcial,impresiones.millaresPorRollo from `tbpro".$procesoActual."` inner join impresiones  where impresiones.id='".$producto."' and `tbpro".$procesoActual."`.longitud!='' and `tbpro".$procesoActual."`.total=1 and `tbpro".$procesoActual."`.tipo='".$tipo."' and `tbpro".$procesoActual."`.producto='".$producto."' and `tbpro".$procesoActual."`.rollo_padre=0 order by `tbpro".$procesoActual."`.noop DESC");
}
...
if($tipo=="1"){
  ...
  $consul=$MySQLiconn->query("SELECT `tbpro".$procesoActual."`.*, bandaspp.idBSPP as newid,(select nombreBanda from bandaseguridad where IDBanda=bandaspp.identificadorBS) as descripcionDisenio,bandaspp.nombreBSPP as descripcionImpresion,bandaspp.anchuraLaminado as alturaEtiqueta from `tbpro".$procesoActual."` inner join bandaspp  where bandaspp.nombreBSPP='".$producto."' and `tbpro".$proceso."`.longitud!='' and `tbpro".$proceso."`.total=1 and `tbpro".$proceso."`.tipo='".$tipo."' and `tbpro".$proceso."`.producto='".$producto."' order by `tbpro".$procesoActual."`.noop DESC");
}
