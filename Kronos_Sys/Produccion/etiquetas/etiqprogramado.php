...
$CK=$MySQLiconn->query("call getTipo(@p0)");
$Rl=$CK->fetch_object();
$tipo=$Rl->id;
...
$MySQLiconn->next_result();
  ...
  $consul=$MySQLiconn->query("SELECT tblotes.*,tbproduccion.juegoLotes,impresiones.id,(select descripcion from producto where ID=impresiones.descripcionDisenio) as descripcionDisenio,impresiones.descripcionImpresion,impresiones.anchoPelicula,impresiones.anchoPelicula as anchura,impresiones.anchoEtiqueta,impresiones.espaciofusion  from tbproduccion inner join tblotes on tbproduccion.juegoLotes=tblotes.juegoLote inner join impresiones on tbproduccion.nombreProducto=impresiones.id where tbproduccion.nombreproducto='$producto' and tbproduccion.estado=2 and tblotes.estado=1 and tblotes.tipo='$tipo'");
}
...
$MySQLiconn->next_result();
 ...
 $consul=$MySQLiconn->query("SELECT lotes.idLote,lotes.longitud,lotes.anchuraBloque,lotes.peso,(lotes.unidades/1000) as unidades,lotes.noop,lotes.tarima,lotes.numeroLote,lotes.referenciaLote,tbproduccion.juegoLotes,bandaspp.idBSPP as id,(SELECT nombreBanda FROM bandaseguridad WHERE IDBanda=bandaspp.identificadorBS) as descripcionDisenio,bandaspp.nombreBSPP as descripcionImpresion,bandaspp.anchuraLaminado as anchura from tbproduccion inner join tblotes lotes on tbproduccion.juegoLotes=lotes.juegoLote inner join bandaspp on tbproduccion.nombreProducto=bandaspp.IdBSPP where tbproduccion.nombreproducto='".$producto."' and tbproduccion.estado=2 and lotes.estado=1 and lotes.tipo='".$tipo."'");
