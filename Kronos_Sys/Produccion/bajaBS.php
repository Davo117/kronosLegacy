...
<center>
<div id="formularius">
<form onsubmit="fusionarBandas();" id="formulary" name="formulary">
<p style="font-size:25px;font-family: Sansation Light;padding-right:500px;padding-top:10px;padding-bottom:5px;border-bottom: 5px solid #476F89;"   id="titulo">Baja de banda de seguridad<b></p><br>
</p>
...
<p>Seleccione el empleado</p>
 <select name="cmbEmpleados" id="cmbEmpleados" class="inputss" required title="Seleccione un empleado" onchange="document.formulary.submit()">
        <option value="">--</option>
        <?php
        $Em=$MySQLiconn->query("SELECT concat(' [',numemple,'] ',nombre,' ',apellido) as nombre,concat(nombre,' ',apellido,'|',numemple) as numEmpleado,numemple from empleado");
        while($rai=$Em->fetch_array())
        {
          
          if(isset($_GET['cmbEmpleados']))
          {
            if($_GET['cmbEmpleados']==$rai['numemple'])
            {
                  ?>
            <option  selected value="<?php echo $rai['numemple'];?>"><?php echo $rai['nombre'];?></option>
            <?php 
            }
              else
          {
                   ?>
            <option value="<?php echo $rai['numemple'];?>"><?php echo $rai['nombre'];?></option>
            <?php
          }
          }
           else
	...
	<a title="Descartar discos" href="Controlador_produccion/crud_unionBS.php?salto=X&cmbEmpleados=<?php echo $_GET['cmbEmpleados'];?>" style="height:30px;width:80px;float:left;margin-right:30px;width:4%;border-radius:3px;" onClick="return confirm('¿Desea continuar?');" name="find"><img src="../pictures/uncheck.png"></img></a>
