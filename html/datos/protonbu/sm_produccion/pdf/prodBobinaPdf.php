<?php
  ...
  if ($_GET['sttbobina'] == 1) { $_SESSION['status_bobina'] = 'Fusionado'; }
  if ($_GET['sttbobina'] == 6) { $_SESSION['status_bobina'] = 'Revisado'; }
  if ($_GET['sttbobina'] == 7) { $_SESSION['status_bobina'] = 'Liberado'; }
  if ($_GET['sttbobina'] == 8) { $_SESSION['status_bobina'] = 'Pesado'; }
  ...
  $prodRolloSelect = $link_mysqli->query("
    ...
    WHERE (prodlote.cdglote = prodbobina.cdglote) AND     
      prodbobina.sttrollo = '".$_GET['sttbobina']."' AND
  ...
  { class PDF extends FPDF
    ...
    { if ($prodDocumento_impresion == '')
      ...
      $prodDocumento_cantidad += (($regProdSubLote->longitud*$pdtoImpresion_alpaso)/$pdtoImpresion_corte);
    ...
    $pdf->Output('Inventario de bobinas '.$_SESSION['prodrollo_producto'].' en '.$_SESSION['status_bobina'].' '.date('Y-m-d').'.pdf', 'I');
  } else
  { echo '<html>
  <head>    
