...
/*
    ...
    { $link_mysqli = conectar();
      $pdtoImpresionSelect = $link_mysqli->query("
        ...
        WHERE pdtoimpresion.cdgimpresion = pdtomezcla.cdgimpresion AND 
          pdtomezcla.cdgmezcla = '".$regProdLote->cdgmezcla."'");
      ...
      { $regPdtoImpresion = $pdtoImpresionSelect->fetch_object();
        ...
        $link_mysqli->query("
          ...
          WHERE cdglote = '".$regProdLote->cdglote."'"); 
      }
    }
  } //*/
  ...
  { $tablero_fchprograma[$idfchprograma] =  $regProdLote->fchprograma; 
    ...
    { $tablero_piezas[$idfchprograma][$regProdLoteDetalle->cdgproceso][$idmezcla] =  $regProdLoteDetalle->piezas;
      $tablero_lotes[$idfchprograma][$regProdLoteDetalle->cdgproceso][$idmezcla] =  $regProdLoteDetalle->lotes;
    ...
    { echo '       
      ...
      { echo '
        ...
        { if ((int)$tablero_piezas[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla] > 0) 
          { echo '
              <tr><td align="left">'.$tablero_proyecto[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].'<br/>
                '.$tablero_impresion[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].'<br/>
                '.$tablero_mezcla[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].'</td>
                <td align="right">
                  <strong>[ '.$tablero_lotes[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].']</strong> '.number_format($tablero_piezas[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla],3).'<br/>
                  <a href="excel/prodLote.php?fchprograma='.$tablero_fchprograma[$idfchprograma].'&cdgmezcla='.$tablero_cdgmezcla[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].'&status='.$prodProceso_proceso[$id_proceso].'">'.$jpg_excel.'</a>
                  <a href="pdf/prodLoteBC.php?fchprograma='.$tablero_fchprograma[$idfchprograma].'&cdgproceso='.$prodProceso_cdgproceso[$id_proceso].'&cdgmezcla='.$tablero_cdgmezcla[$idfchprograma][$prodProceso_cdgproceso[$id_proceso]][$idmezcla].'&status='.$prodProceso_proceso[$id_proceso].'" target="_blank">'.$png_barcode.'</a></td></tr>'; }
        }
  ...
  echo '   
      ...
      <tfoot>
        <tr>
          <td colspan="'.($numprocesos+1).'" align="right">
