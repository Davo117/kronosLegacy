...
<html>
  ...
  { $sistModulo_permiso = sistPermiso($sistModulo_cdgmodulo, $_SESSION['cdgusuario']);
    $prodLote_cdgmezcla = $_POST['slc_cdgmezcla'];
    $prodLote_fchprograma = trim($_POST['txt_fchprograma']);
    $prodLote_amplitud = $_POST['text_amplitud'];
    $prodLote_tolerancia = $_POST['text_tolerancia'];

    if ($prodLote_fchprograma == '')
    { $prodLote_fchprograma = date('Y-m-d'); }
    else
    { $fchprograma = str_replace("-", "", $prodLote_fchprograma);
      ...
      { $prodLote_fchprograma = $ano.'-'.$mes.'-'.$dia; }
      else
      { $prodLote_fchprograma = date('Y-m-d'); }
    }
    ...
    { if (substr($sistModulo_permiso,0,1) == 'r')
      { $link_mysqli = conectar();
        ...
        { $regProdLote = $prodLoteSelect->fetch_object();
          ...
          $prodLote_cdglote = $regProdLote->cdglote;
          $prodLote_fchprograma = $regProdLote->fchprograma;
          $prodLote_longitud = $regProdLote->longitud;
          $prodLote_noop = $regProdLote->noop;
          $prodLote_cdgmezcla = $regProdLote->cdgmezcla;
          $prodLote_sttlote = $regProdLote->sttlote;

          if ($_GET['proceso'] == 'update')
          { if (substr($sistModulo_permiso,0,2) == 'rw')
            { if ($prodLote_sttlote == '1')
              { $prodLote_newsttlote = '0'; }

              if ($prodLote_sttlote == '0')
              { $prodLote_newsttlote = '1'; }

              $link_mysqli = conectar();
              $link_mysqli->query("
                UPDATE prodlote
                SET sttlote = '".$prodLote_newsttlote."'
                WHERE cdglote = '".$prodLote_cdglote."'");
            ...
            { $link_mysqli = conectar();
              ...
              { $link_mysqli = conectar();
                ...
                $pdtoImpresionSelect = $link_mysqli->query("
                  ...
                  AND pdtomezcla.cdgmezcla = '".$prodLote_cdgmezcla."'");
                ...
                $pdtoConsumoSelect = $link_mysqli->query("
                  ...
                  WHERE cdgmezcla = '".$prodLote_cdgmezcla."'");
                ...
                { while ($regPdtoConsumo = $pdtoConsumoSelect->fetch_object())
                  { $pdtoConsumo_cdgproceso = $regPdtoConsumo->cdgproceso;
                    ...
                    VALUES
                       ('".$prodLote_cdglote."', '".$pdtoConsumo_cdgelemento."', '".$prodLote_cdgmezcla."', '".$pdtoConsumo_cdgproceso."', '".$pdtoConsumo_cdgmateria."', '".((($prodLote_longitud*$pdtoImpresion_alpaso)/$pdtoImpresion_corte)*$pdtoConsumo_consumo)."')"); }
                }
          ...
          { if (substr($sistModulo_permiso,0,3) == 'rwx')
            { $link_mysqli = conectar();
              ...
              if ($prodLoteOpeSelect->num_rows > 0)
              { $msg_alert = 'La NoOP '.$prodLote_noop.' NO fue eliminado de la programacion por que ya fue afectado en Produccion.'; }
              else
              { $link_mysqli = conectar();
                $link_mysqli->query("
                ...
                { $link_mysqli = conectar();
                  ...
                  if ($link_mysqli->affected_rows > 0)
                  { $msg_alert = 'NoOp '.$prodLote_noop.' eliminada \n'; }                  
                }
                else
                { $msg_alert = 'NoOp '.$prodLote_noop.' NO fue eliminada \n'; }
              }
            }
            else
    ...
    { if (substr($sistModulo_permiso,0,2) == 'rw')
      { $link_mysqli = conectar();
        $progLoteSelect = $link_mysqli->query("
          ...
          AND (proglote.amplitud BETWEEN ".$prodLote_amplitud." 
            AND ".$prodLote_tolerancia.")
        ...
        { // Busqueda de la impresion
          ...
          { if (isset($_REQUEST['chk_'.$regProgLote->cdglote]))
            { $prodLote_cdglote = $regProgLote->cdglote;
              $prodLote_longitud = $regProgLote->longitud;
              $prodLote_peso = $regProgLote->peso;
              ...
              $prodLote_noopmax = (int)$regProdLoteMax->noopmax;
              $prodLote_noopnum = (int)$regProdLoteMax->noopnum;
              ...
              if ($prodLote_noopmax == $prodLote_noopnum)
              { $prodLote_noop = $prodLote_noopmax+1;
                ...
                $link_mysqli->query("
                  ...
                  VALUES
                    ('".$prodLote_cdglote."', '00', '".$prodLote_longitud."', '".$prodLote_peso."', NOW())");
                ...
                $link_mysqli->query("
                  ...
                  VALUES
                    ('".$prodLote_cdglote."', '".$prodLote_noop."', '".$prodLote_cdgmezcla."', '".$prodLote_longitud."', '".$prodLote_peso."', '".$prodLote_fchprograma."', NOW())");
              }
              else
              { for ($noop = 1; $noop < $prodLote_noopmax; $noop++)
                { $link_mysqli = conectar();
                  $link_mysqli->query("
                  INSERT INTO prodproclote
                    (cdglote, cdgproceso, longitud, peso, fchmovimiento)
                  VALUES
                    ('".$prodLote_cdglote."', '10', '".$prodLote_longitud."', '".$prodLote_peso."', NOW())");

                  $link_mysqli = conectar();
                  $link_mysqli->query("
                  INSERT INTO prodlote
                    (cdglote, noop, cdgmezcla, longitud, peso, cdgproceso, fchprograma, fchmovimiento)
                  VALUES
                    ('".$prodLote_cdglote."', '".$noop."', '".$prodLote_cdgmezcla."', '".$prodLote_longitud."', '".$prodLote_peso."', '10', ".$prodLote_fchprograma."', NOW())");

                  if ($link_mysqli->affected_rows > 0)
                  { $prodLote_noop = $noop;
                    $noop = $prodLote_noopmax; }
                }
              }
              //////////////////////////////////////////////////////////////
              ...
              { $link_mysqli = conectar();
                ...
                if ($link_mysqli->affected_rows > 0)
                { $msg_alert .= 'NoOp '.$prodLote_noop.' generada. \n';
                  ...
                  { $link_mysqli = conectar();
                    ...
                    INSERT INTO prodconsumo
                      (cdglote, cdgelemento, cdgmezcla, cdgproceso, cdgmateria, consumo)
                    VALUES
                      ('".$prodLote_cdglote."', '".$pdtoConsumo_cdgelemento[$idconsumo]."', '".$prodLote_cdgmezcla."', '".$pdtoConsumo_cdgproceso[$idconsumo]."', '".$pdtoConsumo_cdgmateria[$idconsumo]."', '".((($prodLote_longitud*$pdtoImpresion_alpaso)/$pdtoImpresion_corte)*$pdtoConsumo_consumo[$idconsumo])."')"); }
                }
              }
              else
              { $msg_alert .= 'NoOp '.$prodLote_noop.' NO generada. \n'; }
            }
          }
        }
      }
    ...
    echo '
      <form id="form_prodlote" name="form_prodlote" method="POST" action="prodAsignacion.php">
        <table align="center">
          <thead>
            <tr><th colspan="2" align="left">'.$sistModulo_modulo.'</th></tr>
          </thead>
            <tr><td>
                <label for="lbl_cdgmezcla"><a href="../sm_producto/pdtoMezcla.php?cdgmezcla='.$prodLote_cdgmezcla.'">Mezcla</a></label><br/>
                <select style="width:240px" id="slc_cdgmezcla" name="slc_cdgmezcla" onchange="document.form_prodlote.submit()">';
    ...
    { echo '
      ...
      { for ($idmezcla = 1; $idmezcla <= $idsmezcla[$idproyecto][$idimpresion]; $idmezcla++)
        { echo '
          ...
          if ($prodLote_cdgmezcla == $pdtoMezcla_cdgmezcla[$idproyecto][$idimpresion][$idmezcla])
          { echo ' selected="selected"';

            $prodLote_amplitud = $pdtoImpresion_amplitud[$idproyecto][$idimpresion];
            $prodLote_tolerancia = $pdtoImpresion_tolerancia[$idproyecto][$idimpresion]; }
    ...
    echo '
              ...
              <td><label for="lbl_fchprograma">Programa</label><br/>
                <input type="date" style="width:100px;" maxlength="24" id="txt_fchprograma" name="txt_fchprograma" value="'.$prodLote_fchprograma.'" title="Fecha de asignacion" onchange="document.form_prodlote.submit()" required/>
                <input type="hidden" id="text_amplitud" name="text_amplitud" value="'.$prodLote_amplitud.'" />
                <input type="hidden" id="text_tolerancia" name="text_tolerancia" value="'.$prodLote_tolerancia.'" /></td></tr>
            <tr><td colspan="2"><label for="lbl_tolerancia">Rango de amplitud: <strong>'.$prodLote_amplitud.' / '.$prodLote_tolerancia.'</strong></label></td></tr>
          <tbody>
          <tfoot>
    ...
    $prodLoteSelect = $link_mysqli->query("
      ...
      AND prodlote.fchprograma = '".$prodLote_fchprograma."'
    ...
    $progLoteSelect = $link_mysqli->query("
      ...
      FROM progbloque,
        proglote
      WHERE progbloque.cdgbloque = proglote.cdgbloque
      AND (proglote.amplitud BETWEEN ".$prodLote_amplitud." AND ".$prodLote_tolerancia.")
      AND proglote.sttlote = '8'
      ORDER BY proglote.amplitud,
        progbloque.fchbloque,
        proglote.tarima,
    ...
    { echo '
        <table align="center">
          <thead>
            <tr><th colspan="14">Bobinas asignadas a la fecha</th></tr>
      ...
      { echo '
            <tr align="right">
              ...
              <td>'.number_format($prodLotes_longitud[$id_lote],2).' <strong>mts</strong></td>
              <td>'.$prodLotes_espesor[$id_lote].' <strong>micras</strong></td>
              <td>'.$prodLotes_encogimiento[$id_lote].' <strong>%</strong></td>
              <td>'.number_format($prodLotes_peso[$id_lote],3).' <strong>kgs</strong></td>
        ...
        { echo '
              <td align="center"><a href="prodAsignacion.php?cdglote='.$prodLotes_cdglote[$id_lote].'&proceso=consumo">'.$png_button_blue_repeat.'</a></td>
              <td align="center"><a href="prodAsignacion.php?cdglote='.$prodLotes_cdglote[$id_lote].'&proceso=update">'.$png_power_blue.'</a></td>'; }
        else
        { echo '
              <td align="center"><a href="prodAsignacion.php?cdglote='.$prodLotes_cdglote[$id_lote].'&proceso=delete">'.$png_recycle_bin.'</a></td>
              <td align="center"><a href="prodAsignacion.php?cdglote='.$prodLotes_cdglote[$id_lote].'&proceso=update">'.$png_power_black.'</a></td>'; }
      ...
      echo '
          ...
          <tfoot>
            <tr><th colspan="14" align="right"><label for="lbl_ppgdatos">['.$numprodlotes.'] Bobinas programadas</label></th></tr>
          </tfoot>
        </table><br/>'; }
    ...
    { echo '        
        <table align="center">
          <thead>
            <tr><th colspan="9">Bobinas compatibles disponibles</th></tr>
            <tr align="left">
    ...
    echo '
          ...
          <tfoot>
            <tr><th colspan="9" align="right"><label for="lbl_ppgdisponibilidad">['.$numproglotes.'] Bobinas disponibles</label></th></tr>
          </tfoot>
        </table>'; }
