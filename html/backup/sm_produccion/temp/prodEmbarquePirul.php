...
<html>
  ...
  { $link_mysqli = conectar();
    ...
    { $regProdLote = $prodLoteSelect->fetch_object();
      ...
      { if ($progPrograma_sttlote == '1')
        { $progPrograma_newsttlote = '0'; }
      
        if ($progPrograma_sttlote == '0')
        { $progPrograma_newsttlote = '1'; }
      ...
      { $link_mysqli = conectar();
        $prodLoteOpesSelect = $link_mysqli->query("
          SELECT * FROM prodloteopes
          WHERE cdglote = '".$progPrograma_cdglote."'
          AND cdgoperacion NOT LIKE '00090'");

        if ($prodLoteOpesSelect->num_rows > 0)
          ...
          { for ($noop = 1; $noop < $progPrograma_noopmax; $noop++) 
            { $link_mysqli = conectar();
              ...
              INSERT INTO prodproclote
                (cdglote, cdgproceso, longitud, peso, fchmovimiento)
              VALUES
                ('".$progPrograma_cdglote."', '10', '".$progPrograma_longitud."', '".$progPrograma_peso."', NOW())"); 
              ...
              VALUES
                ('".$progPrograma_cdglote."', '".$noop."', '".$progPrograma_cdgmezcla."', '".$progPrograma_longitud."', '".$progPrograma_peso."', '10', ".$progPrograma_fchprograma."', NOW())");  
  ...
  echo '
    <form id="frm_progprograma" name="frm_progprograma" method="POST" action="prodAsignacion.php">
      <table align="center">
        <thead>
