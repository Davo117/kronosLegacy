<?php
  ...
  { $regPdtoImpresion = $pdtoImpresionSelect->fetch_object();
    ...
    $_SESSION['prodrollo_diseno'] = $regPdtoImpresion->diseno;
    $_SESSION['prodrollo_impresion'] = $regPdtoImpresion->impresion; 
    $pdtoImpresion_corte = $regPdtoImpresion->alto;
    $pdtoImpresion_cdgimpresion = $regPdtoImpresion->cdgimpresion; }

  if ($_GET['sttrollo'] == 1) { $_SESSION['status_rollo'] = 'Fusionado'; }
  if ($_GET['sttrollo'] == 6) { $_SESSION['status_rollo'] = 'Revisado'; }
  if ($_GET['sttrollo'] == 7) { $_SESSION['status_rollo'] = 'Liberado'; }
  if ($_GET['sttrollo'] == 8) { $_SESSION['status_rollo'] = 'Pesado'; }
  ...
  $prodRolloSelect = $link_mysqli->query("
    ...
    WHERE (prodlote.cdglote = prodbobina.cdglote AND
      ...
      prodrollo.cdgproducto = '".$pdtoImpresion_cdgimpresion."' AND
  ...
  { class PDF extends FPDF
    { function Header()
      { if ($_SESSION['usuario'] == '')
        ...
        $this->Cell(0,4,utf8_decode('Inventario en rollos: '.$_SESSION['prodrollo_impresion'].' ('.$_SESSION['prodrollo_diseno'].')'),0,1,'R');
        $this->Cell(0,4,'Status de rollo: '.$_SESSION['status_rollo'],0,1,'R');          
        $this->Ln(2); 
        ...
        $this->Cell(35,4,utf8_decode('Código rollo'),0,0,'L',true);
    ...
    { if ($prodDocumento_impresion == '')
      ...
      $pdf->SetFont('arial','',8);
      $pdf->Cell(20,4,'',1,0,'L');
      $pdf->Cell(35,4,$regProdSubLote->cdgrollo,1,0,'R');
      $pdf->Cell(35,4,$regProdSubLote->fchmovimiento,1,0,'L');
      $pdf->Cell(30,4,$regProdSubLote->noop,1,0,'R');
    ...
    $pdf->Output('Inventario de rollos '.$_SESSION['prodrollo_producto'].' en '.$_SESSION['status_rollo'].' '.date('Y-m-d').'.pdf', 'I');
  } else
  { echo '<html>
  <head>    
