<?php
  ...
  { $pdf=new FPDF('P','mm','lbl4x2'); 
    ...
    $pdf->Cell(40,12,'Q'.$alptEmpaque_noempaque,0,1,'R'); 
   ...
   // $pdf->Cell(18,4,'Producto',0,1,'L');
    ...
    $pdf->Cell(58,4,$packingListSelect->num_rows.' Rollos',0,1,'R');    
...
/*
    ...
    $pdf->Cell(18,4,'Peso ',0,0,'L');
    $pdf->SetFont('Arial','B',11);
    $pdf->Cell(40,4,number_format($alptEmpaque_peso,3).' Kgs',0,1,'R');//*/
    ...
    $pdf->Output(); //$pdf->Output($alptEmpaque_impresion.' Empaque Q'.$alptEmpaque_noempaque.' '.$_GET['cdgempaque'].'.pdf', 'I');
  } else
  { echo '<html>
  <head>    
