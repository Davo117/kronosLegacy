...
<html>
  ...
  $prodLoteSelect = $link_mysqli->query("
    SELECT prodlote.cdglote, prodlote.noop
    FROM proglote, prodlote
  ...
  { $progLote_vertodos = 'checked'; 
    ...
    { $link_mysqli = conectar();
      ...
      { $progLotes_idlote[$idtarima][$idlote] = $regProgLote->idlote;
        ...
        $progLotes_fchmovimiento[$idtarima][$idlote] = $regProgLote->fchmovimiento;
  ...
  echo '
          ...
          <tr><td>
            ...
            <td>
              ...
              <input type="text" style="width:100px;" maxlength="16" id="txt_tarima" name="txt_tarima" value="'.$progLote_tarima.'" title="Tarima contenedora" required /></td></tr>
        <tbody>
        <tfoot>
      ...
      <table align="center">
        <thead>
          <tr><td colspan="9"></td>
            <th colspan="2"><input type="checkbox" name="chk_vertodos" id="chk_vertodos" onclick="document.frm_proglote.submit()" '.$progLote_vertodos.'> Ver todo</th></tr>
          <tr align="left">
    ...
    { for ($idlote = 1; $idlote <= $idslote[$idtarima]; $idlote++)
      { echo '
          <tr align="right">
            ...
            <td><strong>'.$prodLote_noop[$progLotes_cdglote[$idtarima][$idlote]].'</strong></td>
            <td>'.number_format($progLotes_longitud[$idtarima][$idlote],2).' mts</td>
            <td>'.number_format($progLotes_amplitud[$idtarima][$idlote]).' mm</td>
            <td>'.number_format($progLotes_espesor[$idtarima][$idlote]).' micras</td>
            <td>'.$progLotes_encogimiento[$idtarima][$idlote].' %</td>
            <td>'.number_format($progLotes_peso[$idtarima][$idlote],3).' kgs</td>
            <td>'.number_format((($progLotes_longitud[$idtarima][$idlote]/$progLotes_peso[$idtarima][$idlote])*($progLotes_amplitud[$idtarima][$idlote]/1000)),3).' mts2</td>';
        ...
        { if ((int)$progLotes_sttlote[$idtarima][$idlote] > 1)
          { echo '
            ...
            <td align="center">'.$progLotes_fchmovimiento[$idtarima][$idlote].'</td>'; 
          } else        
          { echo '
            <td align="center"><a href="progLote.php?cdglote='.$progLotes_cdglote[$idtarima][$idlote].'">'.$png_search.'</a></td>
        ...
        $progLote_sumalongitud[$idtarima] += $progLotes_longitud[$idtarima][$idlote];
        $progLote_sumapeso[$idtarima] += $progLotes_peso[$idtarima][$idlote]; 
        $progLote_sumarendimiento[$idtarima] += (($progLotes_longitud[$idtarima][$idlote]/$progLotes_peso[$idtarima][$idlote])*($progLotes_amplitud[$idtarima][$idlote]/1000)); }

      echo '
          <tr align="right">
            <td></td>
            <th colspan="2">Tarima '.$progLotes_tarima[$idtarima].' : '.$idslote[$idtarima].' Unidades</th>
            <th colspan="2">'.number_format($progLote_sumalongitud[$idtarima],2).' Mts</th>
            <th colspan="3">'.number_format($progLote_sumapeso[$idtarima],3).' Kgs</th>
            <th>'.number_format(($progLote_sumarendimiento[$idtarima]/$idslote[$idtarima]),3).' Mts2</th>
            <td colspan="2"></td></tr>';
