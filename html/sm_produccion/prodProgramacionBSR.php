...
<html>
  <head>
    <meta charset="UTF-8" />
    <title>Programación del Refilado</title>    
    <link rel="stylesheet" type="text/css" href="../css/2014.css" media="screen" />
  </head>
  <body>
    <div id="contenedor">
      <section>
        <a href="ayudaBS.php#progrefilado"><img id="imagen_ayuda" src="../img_sistema/help_blue.png" border="0"/></a>
  ...
  { if ($_GET['mode']=='logout') { cl0s3(); }
    ...
    { echo '
      <div id="loginform">
        <form id="login" action="prodProgramacionBSR.php" method="POST">';
        ...
        { while ($regProgLote = $progLoteSelect->fetch_object())
          { if (isset($_REQUEST['chck'.$regProgLote->cdglote]))
            { $prodProgramacion_cdglote = $regProgLote->cdglote;
              ...
              $prodProgramacion_amplitud = $regProgLote->anchura;
              $prodProgramacion_peso = $regProgLote->peso;              

              $prodProgramacion_programado = (($prodProgramacion_amplitud/$prodProgramacion_anchura)*$prodProgramacion_longitud);
              ...
              { $prodProgramacion_noop = $prodProgramacion_noopmax+1;
                ...
                $link->query("
                  ...
                  VALUES
                    ('".$prodProgramacion_cdglote."', 'BS".date('y')."', '".$prodProgramacion_noop."', '".$prodProgramacion_cdgproducto."', '".$prodProgramacion_longitud."', '".$prodProgramacion_amplitud."', '".$prodProgramacion_peso."', '".$prodProgramacion_fchprograma."', '".$prodProgramacion_programado."', NOW())");
              } else
              ...
              $link->query("
                INSERT INTO prodloteope
                  (cdglote, cdgoperacion, cdgempleado, cdgmaquina, longitud, longitudfin, amplitud, amplitudfin, peso, pesofin, fchoperacion, fchmovimiento)
                VALUES
                  ('".$prodProgramacion_cdglote."', '10001', '".$_SESSION['cdgusuario']."', '".$prodProgramacion_cdgmaquina."', '".$prodProgramacion_longitud."', '".$prodProgramacion_longitud."', '".$prodProgramacion_amplitud."', '".$prodProgramacion_amplitud."', '".$prodProgramacion_peso."', '".$prodProgramacion_peso."', NOW(), NOW())");
    ...
    $progBloqueSelect = $link->query("
      SELECT prodlote.cdgproducto,
       ...
       COUNT(prodlote.cdglote) AS nlotes,
         ...
         SUM(prodloteope.longitudfin) AS longitud,
    ...
    echo '
      <form id="frmProgramacion" name="frmProgramacion" method="POST" action="prodProgramacionBSR.php">
        <div class="bloque">
          <article class="subbloque">
            <label class="modulo_nombre">Programación del Refilado</label>
          </article>         
      ...
      { echo '
          <section class="listado">
            ...
            <article style="vertical-align:top">
              <a href="pdf/prodLotesBS.php?cdgproducto='.$progBloque_cdgproducto[$item].'&cdgmaquina='.$progBloque_cdgmaquina[$item].'&fchprograma='.$prodProgramacion_fchprograma.'" target="_blank">'.$_barcode.'</a>
            </article>
        ...
        { echo '
          ...
          { if ($progLote_sttlote[$item][$subItem] == 'A')
            { echo '
              <a href="prodProgramacionBSR.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=update">'.$_power_blue.'</a>'; }
          
            if ($progLote_sttlote[$item][$subItem] == 'D')
            { echo '
              <a href="prodProgramacionBSR.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=delete">'.$_recycle_bin.'</a>
              <a href="prodProgramacionBSR.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=update">'.$_power_black.'</a>'; }
