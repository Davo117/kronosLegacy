    ...
    {  if (trim($_POST['text_rollo']) != '')
      { $prodRollo_cdgrollo = trim($_POST['text_rollo']);
        ...
        { $regProdRollo = $prodRolloSelect->fetch_object();
          ...
          $prodRollo_rollo = $regProdRollo->rollo;
          $prodRollo_tolerancia = ($prodRollo_rollo*($regProdRollo->tolerancia/100));
            
          $prodRollo_noop = $regProdRollo->noop;
          $prodRollo_minmlles = ($regProdRollo->rollo-$prodRollo_tolerancia);
          $prodRollo_alto = $regProdRollo->alto;
          $prodRollo_maxmlles = ($regProdRollo->rollo+$prodRollo_tolerancia);
          ...
          { if (is_numeric($_POST['text_longitud']))
            { $prodRollo_longitudrollo = $_POST['text_longitud'];
              ...
              { $msg_alert = 'La longitud ingresada es menor a la Mínima (<b>'.($prodRollo_minmlles*$prodRollo_alto).'</b>) especificada para este producto.';
                $text_longitud = 'autofocus';
              } else
              { if (($prodRollo_longitudrollo/$prodRollo_alto) > $prodRollo_maxmlles) 
                { $msg_alert = 'La longitud ingresada es mayor a la Máxima (<b>'.($prodRollo_maxmlles*$prodRollo_alto).'</b>) especificada para este producto.';
                  $text_longitud = 'autofocus';
                } else
                { $text_amplitud = 'autofocus';
                  ...
                  { $prodRollo_amplitudrollo = $_POST['text_amplitud'];
                    ...
                    { $msg_alert = 'Favor de ingresar la información de NUEVO PESO.';
                      $text_peso = 'autofocus'; } 
                  } else
                  { $msg_alert = 'Favor de ingresar la información de NUEVA AMPLITUD.';
                    $text_amplitud = 'autofocus'; } 
                }
              } 
            } else
            { $msg_alert = 'Favor de ingresar la información de NUEVA LONGITUD.';
              $text_longitud = 'autofocus'; } 
          } else
          { $msg_alert = 'Producto bloqueado para empacarse como rollo.'; }
        } 

        $text_rollo = 'autofocus'; 
      } else 
      { $msg_alert = 'Favor de ingresar la información de Rollo.';
        $prodRollo_cdgrollo = '';
        $text_rollo = 'autofocus'; }
    } else 
    { $msg_alert = $msg_alert; } 
