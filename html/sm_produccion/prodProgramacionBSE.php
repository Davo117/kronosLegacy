...
<html>
  <head>
    <meta charset="UTF-8" />
    <title>Programación del Embosado</title>    
    <link rel="stylesheet" type="text/css" href="../css/2014.css" media="screen" />
  </head>
  <body>
    <div id="contenedor">
      <section>
        <a href="ayudaBS.php#progembosado"><img id="imagen_ayuda" src="../img_sistema/help_blue.png" border="0"/></a>
  ...
  { if ($_GET['mode']=='logout') { cl0s3(); }
    ...
    { echo '
      <div id="loginform">
        <form id="login" action="prodProgramacionBSE.php" method="POST">';
      ...
      { $item = 0;
        ...
        { $item++;
          ...
          { $prodProgramacion_cdgsustrato = $regPdtoBandaP->cdgsustrato; 
            $prodProgramacion_anchura = $regPdtoBandaP->anchura; }
        }
            ...
            { $prodProgramacion_cdglote = $regProgLote->cdglote;
              ...
              { $prodProgramacion_noop = $prodProgramacion_noopmax+1;
                ...
                $link->query("
                  ...
                  VALUES
                    ('".$prodProgramacion_cdglote."', 'BS".$subSerie."', '".$prodProgramacion_noop."', '".$prodProgramacion_cdgproducto."', '".$prodProgramacion_longitud."', '".$prodProgramacion_anchural."', '".$prodProgramacion_peso."', '".$prodProgramacion_fchprograma."', '".$prodProgramacion_programado."', NOW())");
              } else
    ...
    echo '
      <form id="frmProgramacion" name="frmProgramacion" method="POST" action="prodProgramacionBSE.php">
        <div class="bloque">
          <article class="subbloque">
            <label class="modulo_nombre">Programación del Embosado</label>
          </article>         
      ...
      { echo '
          <section class="listado">
            <article align="right" style="vertical-align:top">
              <label><a href="pdf/pro-fr101.php?cdgproducto='.$progBloque_cdgproducto[$item].'&cdgmaquina='.$progBloque_cdgmaquina[$item].'&fchprograma='.$prodProgramacion_fchprograma.'&cdgfchprograma='.$prodProgramacion_cdgfchprograma.'" target="_blank">PRO-<b>FR00</b></a></label><br/>
            </article>
        ...
        { echo '
          ...
          { if ($progLote_sttlote[$item][$subItem] == 'A')
            { echo '
              <a href="prodProgramacionBSE.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=update">'.$_power_blue.'</a>'; }
          
            if ($progLote_sttlote[$item][$subItem] == 'D')
            { echo '
              <a href="prodProgramacionBSE.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=delete">'.$_recycle_bin.'</a>
              <a href="prodProgramacionBSE.php?cdglote='.$progLote_cdglote[$item][$subItem].'&proceso=update">'.$_power_black.'</a>'; }
          ...
          echo '
            ...
            <article>
              <label class="textNombre">NoOP <b>'.$progLote_noop[$item][$subItem].'</b></label><br/>
              <label>'.$progLote_lote[$item][$subItem].'</label><br/>
