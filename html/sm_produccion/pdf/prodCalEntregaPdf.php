<?php
  ...
  { function Header()
    { if ($_SESSION['usuario'] == '')
      ...
      $this->Cell(0,4,utf8_decode('(Producción) Programa de entregas del '.$_GET['dsdfecha'].' al '.$_GET['hstfecha']),0,1,'R');
  ...
  { if ($vntsCalEntrega_cdgsucursal == '' AND $vntsCalEntrega_cdgproducto == '')
    { // Busqueda de compromisos por fecha embarque 
      ...
      { while($regVntsCalEntrega_fechas = $vntsCalEntregaSelect->fetch_object())
        { $pdf->SetFont('arial','',12);
         ...
         DATEDIFF(vntsoclote.fchembarque, vntsoc.fchrecepcion) AS fchplazo
                FROM vntsoc,
                  vntsoclote
                WHERE vntsoc.cdgsucursal = '".$regVntsCalEntrega_sucursales->cdgsucursal."' AND
              ...
              { $pdf->SetFont('arial','B',8);
                ...
                { $pdf->SetFont('arial','B',5); 
                  ...
                  $pdf->Cell(60,4,$vntsCalEntrega_productos[$regVntsCalEntrega_lotes->cdgproducto],1,0,'L');
      ...
      { $pdf->Cell(80,4,$prodEntrega_empaque[$id_empaque],1,1,'L',true); 
        ...
        { if ($prodEntrega_cantidad[$prodEntrega_cdgempaque[$id_empaque]][$prodEntrega_cdgproducto[$id_producto]] > 0) 
          { $pdf->SetFont('arial','',8);
            $pdf->Cell(60,4,$prodEntrega_producto[$id_producto],1,0,'L');
            ...
            $prodEntrega_subtotal[$prodEntrega_cdgempaque[$id_empaque]] += $prodEntrega_cantidad[$prodEntrega_cdgempaque[$id_empaque]][$prodEntrega_cdgproducto[$id_producto]];
            $prodEntrega_total += $prodEntrega_cantidad[$prodEntrega_cdgempaque[$id_empaque]][$prodEntrega_cdgproducto[$id_producto]];
          }          
        }
        $pdf->SetFont('arial','B',9);
        $pdf->Cell(60,5,'Subtotal ',0,0,'R');
        $pdf->Cell(20,5,number_format($prodEntrega_subtotal[$prodEntrega_cdgempaque[$id_empaque]],3),1,1,'R');  
         ...
         DATEDIFF(vntsoclote.fchembarque, vntsoc.fchrecepcion) AS fchplazo
            ...
            { $pdf->SetFont('arial','B',8);
              ...
              { $pdf->SetFont('arial','B',5); 
                ...
                $pdf->Cell(20,4,$regVntsCalEntrega_lotes->oc.'-'.$regVntsCalEntrega_lotes->idlote,1,0,'R');
                $pdf->Cell(25,4,$regVntsCalEntrega_lotes->fchrecepcion.'  ('.$regVntsCalEntrega_lotes->fchplazo.')',1,0,'L'); 
                $pdf->Cell(60,4,$vntsCalEntrega_sucursales[$regVntsCalEntrega_lotes->cdgsucursal],1,0,'L');
                $pdf->Cell(20,4,number_format($regVntsCalEntrega_lotes->cantidad,3),1,0,'R'); 
                $pdf->Cell(30,4,$vntsCalEntrega_empaques[$regVntsCalEntrega_lotes->cdgempaque],1,0,'L'); 
                $pdf->Cell(33,4,$regVntsCalEntrega_lotes->referencia,1,1,'L');
