<?php
  ...
  { $prodDocumento_cdgdocumento = $_GET['cdgdocumento'];  
    $prodDocumento_sttbobina = $_GET['sttbobina'];
      ...
      { $link_mysqli = conectar();
        $pdtoMezclaSelect = $link_mysqli->query("
          ...
          WHERE (prodlote.cdglote = prodbobina.cdglote AND
            ...
            (prodbobina.sttbobina = '7' OR prodbobina.sttbobina = '9')) AND
        ...
        { include '../../fpdf/fpdf.php';
          ...
          { $link_mysqli = conectar();
            $prodSubLoteSelect = $link_mysqli->query("
              ...
              AND ( prodbobina.cdgproducto = pdtoimpresion.cdgimpresion
                AND pdtoimpresion.cdgdiseno = pdtodiseno.cdgdiseno)              
              AND prodbobina.cdgdocumento = '".$prodDocumento_cdgdocumento."'
              AND prodbobina.cdgproducto = '".$regPdtoMezcla->cdgproducto."'
              AND (prodbobina.sttbobina = '7' OR prodbobina.sttbobina = '9')
        ...
        { echo 'El documento que buscas contiene problemas de referencia.'; }

        $pdtoMezclaSelect->close;
      } else
      { echo 'El documento que buscas esta vacio.'; }

      $prodSubLoteSelect->close;
    } else
    { echo 'El documento que buscas no existe.'; }

    $prodDocumentoSelect->close;
  } else
  { echo 'El necesario acceder a los documentos desde el modulo de documentos.'; }
?>
