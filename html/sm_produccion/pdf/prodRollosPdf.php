  ...
  { 
    ...
    { // Cabeza de página
      ...
      { if ($_SESSION['usuario'] == '')
        ...
        $this->Cell(75,4,utf8_decode('Listado de rollos por producto'),0,1,'L');

        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Diseño'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['prodrollo_diseno']),0,1,'L');
      
        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Impresión'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['prodrollo_impresion']),0,1,'L');
      
        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Status'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['status_rollo']),0,1,'L');
    ...
    $item = 0;
    while ($regDataPdf = $prodRolloSelect->fetch_object())
    { $item++;
      ...
      $pdf->Cell(25,4,$regDataPdf->cdgrollo,1,0,'C');
      $pdf->Cell(5,4,'',1,1,'R');
    ...
    $pdf->Output('Listado de rollos de '.$_SESSION['prodrollo_impresion'].' en '.$_SESSION['status_rollo'].' '.date('Y-m-d G:i:s').'.pdf', 'I');
  } else
  { echo '<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    ...
    echo '
      <h1>Listado de rollos por producto, sin resutados que mostrar.</h1>
    </div>
  </body>
</html>'; }
