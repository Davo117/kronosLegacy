<?php
  ...
  { $regPdtoBandaP = $pdtoBandaPSelect->fetch_object();
    ...
    $_SESSION['prodbobina_banda'] = $regPdtoBandaP->banda;
    $_SESSION['prodbobina_bandap'] = $regPdtoBandaP->bandap;
  ...
  $prodBobinaSelect = $link->query("
    ...
    CONCAT(prodlote.serie,'.',prodlote.noop,'-',prodbobina.bobina) AS noop
      FROM prodlote,
           prodbobina,
           pdtobanda,
           pdtobandap
    WHERE (prodlote.cdglote = prodbobina.cdglote AND
           prodbobina.cdgproducto = '".$pdtoBandaP_cdgbandap."' AND
  ...
  { 
    ...
    { // Cabeza de página
      ...
      { if ($_SESSION['usuario'] == '')
        ...
        $this->Cell(75,4,utf8_decode('Listado de bobinas de banda de seguridad'),0,1,'L');

        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Banda'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['prodbobina_banda']),0,1,'L');
      
        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Producto'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['prodbobina_bandap']),0,1,'L');
    ...
    $pdf->Output('Listado de bobinas de '.$_SESSION['prodbobina_banda'].' en '.$_SESSION['status_bobina'].' '.date('Y-m-d G:i:s').'.pdf', 'I');
  } else
  { echo '<!DOCTYPE html>
<html>
  <head>
