<?php
  ...
  { $regPdtoBandaP = $pdtoBandaPSelect->fetch_object();
    ...
    $_SESSION['proddisco_banda'] = $regPdtoBandaP->banda;
    $_SESSION['proddisco_bandap'] = $regPdtoBandaP->bandap;
    $pdtoBandaP_cdgbandap = $regPdtoBandaP->cdgbandap; }

  if ($_GET['sttdisco'] == '1') { $_SESSION['status_disco'] = 'Sliteado'; }  
  
  if ($_GET['preembosado'] == 1)
  { $prodDiscoSelect = $link->query("
      SELECT proddisco.cdgdisco,
             proddisco.longitud,
             proddisco.amplitud,
             proddisco.peso,
             proddisco.fchmovimiento,
      CONCAT(prodlote.serie,'.',prodlote.noop,'-',prodbobina.bobina,'-',proddisco.disco) AS noop
        FROM prodlote,
             prodbobina,
             proddisco,
             pdtobanda,
      ...
      WHERE (pdtobandap.cdgbanda = pdtobanda.cdgbanda AND
            ...
            (prodlote.cdglote = prodbobina.cdglote AND
             ...
             proddisco.cdgproducto = '".$pdtoBandaP_cdgbandap."' AND
             proddisco.sttdisco = '1') AND
             proddisco.longitud > 0
  ...
  { $prodDiscoSelect = $link->query("
      SELECT proddisco.cdgdisco,
             ...
             proddisco.peso,
             proddisco.fchmovimiento,
      CONCAT(prodlote.serie,'.',prodlote.noop,'-',proddisco.disco) AS noop
        FROM prodlote,
             proddisco,
             pdtobanda,
             pdtobandap
  ...
  { 
    ...
    { // Cabeza de página
      ...
      { if ($_SESSION['usuario'] == '')
        ...
        $this->Cell(75,4,utf8_decode('Listado de discos de banda de seguridad'),0,1,'L');

        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Banda'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['proddisco_banda']),0,1,'L');
      
        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Producto'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['proddisco_bandap']),0,1,'L');
      
        $this->SetFont('Arial','B',8);
        $this->Cell(125,4,utf8_decode('Status'),0,0,'R');
        $this->Cell(0.5,4,'',0,0,'R',true);
        $this->SetFont('Arial','I',8);
        $this->Cell(75,4,utf8_decode($_SESSION['status_disco']),0,1,'L');
    ...
    $item = 0;
    while ($regDataPdf = $prodDiscoSelect->fetch_object())
    { $item++;
      ...
      $pdf->Cell(25,4,$regDataPdf->cdgdisco,1,0,'C');
      $pdf->Cell(5,4,'',1,1,'R');
    ...
    $pdf->Cell(20,4,number_format($dataPdf_peso,3,'.',','),1,1,'R');

    $pdf->Output('Listado de discos de '.$_SESSION['proddisco_banda'].' en '.$_SESSION['status_disco'].' '.date('Y-m-d G:i:s').'.pdf', 'I');
  } else
  { echo '<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    ...
    echo '
      <h1>Listado de discos por producto, sin resutados que mostrar.</h1>
    </div>
  </body>
</html>'; }
