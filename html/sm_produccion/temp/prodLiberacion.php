...
<html>
  ...
  { $sistModulo_permiso = sistPermiso($sistModulo_cdgmodulo, $_SESSION['cdgusuario']); 
            ...
            { $progLote_amplitud = $_POST['txt_amplitud'];
              ...
              { $progLote_peso = $_POST['txt_peso']; 
                ...
                $link_mysqli->query("
                  ...
                  VALUES
                    ('".$progLote_cdglote."', '10090', '".$_SESSION['cdgusuario']."', '00000', '".$fchoperacion."', NOW())");

                if ($link_mysqli->affected_rows > 0) { $msg_alert .= 'Operacion registrada. \n'; }
                ...
                if ($link_mysqli->affected_rows > 0) { $msg_alert .= 'Proceso registrado. \n'; }
                ...
                if ($link_mysqli->affected_rows > 0) { $msg_alert .= 'Bobina afectada.'; }
    ...
    echo '
    <form id="frm_progliberacion" name="frm_progliberacion" method="POST" action="prodLiberacion.php">
      <table align="center">
        <thead>
          <tr><th>'.$sistModulo_modulo.'</th></tr>
        </thead>
        <tbody>
          <tr><td><label for="ttl_empleado"><strong>ID </strong>'.$_SESSION['idusuario'].'</label><br/>              
              <label for="info_empleado"><strong>Nombre </strong>'.$_SESSION['usuario'].'</label>              
          <tr><td><label for="ttl_lote">Lote/Bobina</label><br/>
              <input type="text" id="txt_idlote" name="txt_idlote" style="width:200px" maxlenght="24" value="'.$progLote_lote.'" title="Identificador de lote/bobina" '.$txt_lote.' required/><br/>
              <label for="ttl_lote"><strong>ID </strong>'.$progLote_cdglote.'</label></td></tr>
          <tr><td>
              <table align="center">
